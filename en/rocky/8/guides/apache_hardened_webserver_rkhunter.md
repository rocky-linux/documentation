...
_rkhunter_ (Root Kit Hunter) is a Unix-based tool that scans for rootkits, backdoors and possible local exploits. It is a good part of a hardened web server, which theoretically notifies the administrator quickly when something happens on the file system. _rkhunter_ is just one possible component of a hardened Apache web server setup and can be used with or without other tools. If you'd like to use this along with other tools for hardening, refer back to the [Apache Hardened Web Server routine](apache_hardened_webserver.md). This document also uses all of the assumptions and conventions outlined in that original document, so it is a good idea to review it before continuing.
...
The only configuration options that need to be set are those dealing with mailing the report to the administrator. To modify the configuration type:
...
_rkhunter_ can be run by typing it at the command-line. There is a cron job installed for you in `/etc/cron.daily` but if you want to automate the procedure with a different schedule, look at the [Automating cron job procedure](cron_jobs_howto.md), and move the script somewhere other than `/etc/cron.daily`, such as `/usr/local/sbin` and then call it from your custom cron job. The easiest, of course, is to leave the default cron.daily setup in tact.
