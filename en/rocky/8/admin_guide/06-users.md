...
Each user is a member of at least one group: **this is his main group**.
...
> :fire: **DANGER:**
It is recommended to use the administration commands instead of manually editing the files.
...
> :notebook: **NOTE:**
Under **Debian**, the administrator should prefer, except in scripts intended to be portable to all Linux distributions, the `addgroup` and `delgroup` commands as specified in the *man*:
```
[root] # man addgroup
DESCRIPTION
    adduser and addgroup add users or groups to the system according to the options provided on the command line and the information contained in the configuration file /etc/adduser.conf. These are more user-friendly interfaces than the useradd and groupadd programs. They allow you to choose default UIDs or GIDs according to the Debian policy, to create a personal directory configured according to a template (skeleton), to use a custom script, and other features.
```
...
This file contains the group informations (separated by `:`).
...
*A user can belong to several groups in addition to his main group.*
...
When a user is created, his home directory and environment files are created.
