...
Among the existing command interpreters, the **Bourne-Again Shell** (`/bin/bash`) is the one most frequently used. It is assigned by default to new users. For various reasons, advanced Linux users will choose alternative shells from among the Korn Shell (`ksh`), the C Shell (`csh`), etc.
...
To manipulate the history, commands allow from the command prompt to:
...
The `-n` option allows not to return to the line after displaying the text (which is the default behavior of the command).
...
In this last example, the `-d` option displays a given date. The `+%j` option formats this date to show only the quantieme (day of the year).
...
The format to be respected for the argument following the `-s` option is this:
...
* pts/: represents a virtual console in a graphical environment.
...
> :star: **TIP:**
The `touch` command is primarily used to create an empty file, but it can be useful for incremental or differential backups as example. Indeed, the only effect of executing a `touch` on a file will be to force it to be saved during the next backup.
...
Using the <kbd>ENTREE</kbd> key, the move is line by line. Using the <kbd>ESPACE</kbd> key, the move is page by page. `/text` allows you to search for the occurrence in the file.
...
| ---------------- | ------------- |
| `h`              | Help.
| `Flèches`        | Move up, down a line, or to the right or left..
| `Entrée`         | Move down one line.
| `Espace`         | Move down one page.
| `PgAR` ou `PgAV` | Move up or down one page.
| `Pos1` ou `Fin`  | Move to the beginning or end of a file.
...
| `-size taille`      | Search for files by size.
...
> :star: **TIP:**
  **Understand the -exec** option
  ...
  If the `find` command finds three files named `log1.txt`, `log2.txt`, and `log3.txt`, then the `find` command will construct the string by replacing in the string "`rm -f {} \; the braces with one of the results of the search, and this as many times as there are results.
  This will give us :
  ```
...
Meta-characters replace one or more characters (or even an absence of characters) during a search.
...
Metacharacters allow more complex searches by replacing all or part of a word. Simply replace the unknowns with these special characters.
...
> :notebook: **NOTE:**
Always surround words containing metacharacters with `"` to prevent them from being replaced by the names of files that meet the criteria.

> :fire: **DANGER:**
Do not confuse shell metacharacters with regular expression metacharacters. The `grep` command uses regular expression metacharacters.
...
> :fire: **CAUTION:**
The ending keyword, here `END` or `STOP`, to end the command must be the only word on the line and must be at the beginning of its line.
...
$ wc -l < .bash_profile
27 # the number of lines is only
```
...
A **pipe** is a mechanism allowing to link the standard output of a first command to the standard input of a second command.

This communication is monodirectional and is done with the `|` symbol. The pipe symbol `|` is obtained by pressing the <kbd>AltGR</kbd> + <kbd>6</kbd> keys simultaneously.
