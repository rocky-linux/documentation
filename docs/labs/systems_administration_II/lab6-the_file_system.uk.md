...
2. Виконайте команду `losetup` без будь-яких опцій, щоб показати активні пристрої циклу. Впишіть:

    ```bash
    [root@serverPR root]# losetup
    ```

3. Знову виконайте команду `losetup`, щоб переглянути/знайти перший невикористаний пристрій циклу. Впишіть:
...
5. Знову виконайте команду `losetup`, щоб показати пристрої циклу, що використовуються. Впишіть:
...
6. Використайте утиліту `sfdisk` для перегляду всіх розділів на новому псевдоблочному пристрої. Впишіть:
...
7. Тепер використовуйте `fdisk`, щоб переглянути список будь-яких розділів на тому ж пристрої. Впишіть:
...
1. Ви створите новий розділ за допомогою програми `fdisk`. Команда `fdisk` працює інтерактивно, тому у вас буде багато запитань та відповідей для виконання певних завдань.

   Почніть з передачі назви блокового пристрою циклу як аргументу функції `fdisk`. Впишіть:
...
3. Зі списку довідки, що відображається, ми бачимо, що `n` використовується для додавання нового розділу. Введіть `n` у командному рядку:
...
8. Повернувшись у головну підказку `fdisk`, введіть `p`, щоб надрукувати поточну таблицю розділів блокового пристрою:
   ...
   Новий створений вами розділ — це той, що вказаний вище у файлі `/dev/loop0p1`. Ви помітите, що тип розділу «83».

9. Все виглядає добре. Запишіть усі зміни до таблиці розділів, ввівши підкоманду `w` команди `fdisk`:
...
10. Відповідно до попередження, яке ви могли отримати після запису таблиці розділів на диск у попередньому кроці, іноді вам може знадобитися вжити додаткових заходів, щоб спонукати ядро підтвердити нові зміни жорсткого диска. Використайте команду `partprobe`, щоб зробити це:
    ...
    !!! tip
            ...
            Під час використання `fdisk`, тип розділу за замовчуванням для щойно створених розділів — Linux (0x83). Ви можете скористатися командою `fdisk` `t`, щоб змінити тип. Наприклад, щоб змінити тип розділу на тип LVM (0x8e), вам слід виконати такі дії:
            
            Введіть `t`, щоб змінити тип розділу:
            
            `bash         Command (m for help): t         `
              
            Потім введіть шістнадцятковий код (0x8e) для розділів типу LVM у командному рядку:
            
            `bash         Hex code or alias (type L to list all): 8e         `
            
            Запишіть усі зміни до таблиці розділів, ввівши команду `w` `fdisk`:
            
            `bash         Command (m for help): w         `
...
1. Увійшовши в систему як користувач з правами адміністратора, створіть розріджений файл розміром 10 Гб під назвою `10G-fake-lvm-disk.img`. Впишіть:

    ```bash
    [root@server root]# truncate --size 10GiB /tmp/10G-fake-lvm-disk.img
    ```

2. Виконайте команду `losetup`, щоб переглянути/знайти перший невикористаний пристрій циклу. Впишіть:
...
4. Виконайте команду `losetup`, щоб показати пристрої циклу, що використовуються. Впишіть:
...
5. Використовуйте команду `pvdisplay` для перегляду фізичних томів, визначених на даний момент у системі. Впишіть:
...
6. Ініціалізуйте новий блоковий пристрій /dev/loop1 (10G-fake-lvm-disk.img) як фізичний том. Використайте утиліту `pvcreate`. Впишіть:
...
1. Використовуйте команду `vgdisplay` для перегляду поточних налаштованих груп томів. Впишіть:
   ...
   **ВИХІД**
   ...
   !!! note
            ...
            З наведеного вище виводу:
            - Назва групи томів — rl
            - Розмір віртуальної групи становить 98,41 ГіБ.
            - У VG є 0 (нуль) вільних фізичних екстентів (PE), що еквівалентно 0 МБ простору.

2. Призначте новий PV (/dev/loop1) існуючій групі томів `rl`. Використайте команду `vgextend`, введіть:
...
3. Знову виконайте команду `vgdisplay`, щоб переглянути зміни. Впишіть:
   ...
   **ВИХІД**
   ...
   !!! Question "Питання"
        ...
        ```
         Використовуючи вивід `vgdisplay`, запишіть зміни у вашій системі. Які нові значення для "Free PE / Size"?
        ```

4. Щоб видалити новий PV (/dev/loop1) з існуючої групи томів `rl`. Використайте команду `vgreduce`, введіть:
...
У цьому розділі ви створите нову окрему групу томів під назвою "scratch".  Скретч-вібратор отримає весь свій простір з псевдоблочного пристрою `/dev/loop1`.

1. Створіть новий «порожній» простір. Впишіть:
...
2. Виконайте команду `vgdisplay`, щоб переглянути зміни. Впишіть:
    ...
    ```
       ...
       !!! question "Питання"
        ...
        ```
         Перегляньте вивід `vgdisplay`. Які значення для "Free PE / Size"? І чим ці значення відрізняються від попереднього розділу, коли ви додавали PV `/dev/loop1` до існуючої групи томів `rl`?
        ```
...
1. Використовуйте команду `lvdisplay` для перегляду поточних налаштованих логічних томів. Впишіть:
    ...
    ```
       ...
       !!! question "Питання"
        ...
        ```
         На основі ваших результатів дайте відповіді на наступні питання: 
        
         1. Скільки логічних томів (LVs) визначено?
...
2. Використайте команду `lvs` для аналогічного відображення логічних томів, але цього разу відфільтруйте вивід, щоб показати певні поля. Фільтр для перегляду полів lv_name (ім’я логічного тому), lv_size (розмір логічного тому), lv_path, vg_name (ім’я групи томів). Впишіть:
...
3. На новому віртуальному дискі `scratch` створіть новий логічний том під назвою «scratch2» за допомогою команди `lvcreate`. Встановіть розмір для `scratch2` рівним 2 Гб. Впишіть:
...
4. Створіть другий логічний том під назвою «scratch3». Цього разу використайте весь доступний простір, що залишився, на групі томів `scratch`. Впишіть:
...
3. Після видалення відповідних томів ви можете зменшити розмір віртуальної групи `rl`, щоб зробити її узгодженою. Впишіть:

    ```bash
    [root@localhost ~]# vgreduce --removemissing  rl
    ```

4. Видаліть будь-які мітки LVM з PV `/dev/loop1`. Впишіть:
...
1. Використайте утиліту `mkfs.vfat` для створення файлової системи типу vfat на томі `/dev/loop0p1`. Впишіть:
...
2. Використовуйте `lsblk` для запиту до системи цікавої інформації про блочний пристрій. Впишіть:
...
1. Використайте утиліту `mkfs.ext4` для створення файлової системи типу EXT4 на томі `/dev/scratch/scratch2`. Впишіть:
...
2. Використовуйте `lsblk` для запиту до системи цікавої інформації про том scratch2. Впишіть:
...
1. Використайте утиліту `mkfs.xfs` для створення файлової системи типу XFS на томі `/dev/rl/scratch3`. Впишіть:
    ...
    ```
       ...
       !!! question "Питання"
        ...
        ```
         ...
         2. Яке значення максимальної кількості монтованих дисків вашого кореневого тому (/dev/rl/root)?
        ```

2. Налаштуйте/встановіть максимальне значення лічильника монтування на нуль між перевірками файлової системи на томі `/dev/scratch/scratch2`. Використайте команду `tune2fs`. Впишіть:
...
3. Використайте команду `fsck` для перевірки файлової системи scratch2. Впишіть:
...
5. Використовуйте `lsblk` для перегляду інформації про `/dev/scratch/scratch2`. Впишіть:
...
4. Використайте команду `mount` для відображення всіх файлових систем VFAT у системі. Використайте grep для фільтрації виводу за словом `scratch`. Впишіть:

    ```bash
    [root@localhost ~]# mount -t vfat | grep scratch
    ```

5. Використовуйте команду `df` для перегляду звіту про використання дискового простору файлової системи на сервері. Впишіть:

    ```bash
    [root@localhost ~]# df -ht vfat | grep scratch
    ```

6. Використовуйте опцію `--bind` з командою `mount`, щоб файлова система `/mnt/10gb-scratch1-partition` також відображалася під простішою або зручнішою для користувача назвою/шляхом, наприклад, `/mnt/scratch1`. Впишіть:
...
3. Використовуйте команду `mount` для відображення всіх файлових систем EXT4 у системі. Впишіть:
...
5. Використовуйте команду `df` для перегляду звіту про використання дискового простору файлової системи на сервері.

#### Для монтування файлової системи XFS

1. Створіть папку з назвою `/mnt/8gb-scratch3-volume`. Це буде точка монтування для файлової системи scratch3. Впишіть:
...
3. Використовуйте команду `mount` для відображення всіх файлових систем XFS у системі. Впишіть:

    ```bash
    [root@localhost ~]# mount -t xfs | grep scratch
    ```

4. Використовуйте команду `df` для перегляду звіту про використання дискового простору файлової системи на сервері.
...
2. Перш ніж вносити будь-які зміни, зробіть резервну копію файлу `/etc/fstab`. Впишіть:
...
5. Використайте команду `systemctl`, щоб увімкнути нову службу loopdevice. Впишіть:
...
Нерозсудливий користувач випадково помічає, що за ніч на сервері з'явилися нові _**початкові**_ файлові системи. "Це чудово!" — каже він собі.
...
4. Після запуску процесу `dd`, прогуляйтеся та поверніться, коли команда завершиться або коли виникне помилка, оскільки далі не можна продовжити. Або піти і знайти адміністратора та поскаржитися на те, що дисковий простір у системі переповнений.

5. Ознайомтеся з іншими нерозумними/безглуздими/дратівливими речами, які можна робити в системі. Ви _**нерозсудливий користувач**_.
...
2. Перегляньте файл `/etc/fstab` у обраному вами редакторі. Нижче наведено відповідний запис у файлі, перш ніж ми внесемо будь-які зміни у файл.
...
5. Використайте утиліту `sed`, щоб знайти рядок, який ми хочемо змінити, та внести оновлення. Впишіть:
...
7. Щоб зміни до `/etc/fstab` набули чинності, вам потрібно зробити ще кілька речей. Спочатку перезавантажте systemd-daemon, виконавши:
...
9. Перевірте файл `/proc/mounts`, щоб переконатися, що нові параметри монтування застосовано. Впишіть:
   ...
   **ВИХІД**
   ...
   !!! tip
           ...
           Ви також можете перевірити параметри монтування, що використовуються для будь-якої файлової системи, за допомогою команди `mount`. У попередньому прикладі ви можете переглянути параметри монтування для тома scratch2, відформатованого ext4, виконавши команду:
           
            `bash          [root@localhost ~]# mount -t ext4 | grep scratch2          `
            **OUTPUT**
            `         /dev/mapper/scratch-scratch2 on /mnt/2gb-scratch2-volume type ext4   (rw,relatime,quota,usrquota,grpquota)         `

   !!! question

            Запишіть команди для окремого `розмонтування` заданої файлової системи, а потім її зворотного `монтування`.
...
10. Тепер вам потрібно підготувати файлову систему для підтримки квот. Створіть файли квот і згенеруйте таблицю поточного використання диска для кожної файлової системи. Впишіть:
    ...
    **ВИХІД**
    ...
    !!! question
            ...
            Після виконання наведеної вище команди ви помітите два нових файли, створені в каталозі «/mnt/2gb-scratch2-volume». Перелічіть ці файли тут?

    !!! tip

            Щоб отримати актуальний стан файлової системи квот, слід періодично виконувати команду `quotacheck -avcug`, коли квота вимкнена у файловій системі.
...
1. Ви створите обмеження за допомогою прототипу користувача. Користувач під назвою «me» буде вашим прототипом користувача. Використайте команду `edquota` для створення обмежень. Впишіть:
...
2. Ви створите пільговий період за допомогою опції `-t` з командою `edquota`. Впишіть:
...
4. Щоб отримати звіт про стан усіх увімкнених квот, введіть:
   ...
   **ВИХІД**
   ...
   !!! Question "Питання"
        ...
        ```
         Зі значення вище у стовпці пільгового періоду для користувача `unreasonable`, скільки пільгового періоду залишилося у користувача? 
        ```
...
6. Використайте команду `su`, щоб тимчасово прийняти особу користувача `unreasonable` та спробувати створити додаткові файли або каталоги від імені цього користувача. Впишіть:

    ```bash
    [root@localhost ~]# su - unreasonable
    ```

7. Увійшовши в систему як користувач unracial, ви перевіряєте та помічаєте, що файл `/mnt/2gb-scratch2-volume/LARGE-USELESS-FILE.tar`, який ви створили в попередній вправі, відсутній! Роздратований, ви вирішуєте створити його знову. Впишіть:
...
10. Розчаровані невіглаством, нерозумні користувачі типи:
    ...
    !!! Note "Примітка"
        ...
        ```
        «Нерозумний» користувач буде змушений щось зробити зі створеним ним файлом «LARGE-UNELESS-FILE.tar». Доки цей користувач не зменшить загальний розмір файлу до ліміту, він не зможе зробити багато іншого. 
        ```
