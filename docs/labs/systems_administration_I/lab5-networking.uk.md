...
3. Виконайте команду, щоб знову ідентифікувати пристрій, але збережіть повернуте значення в змінній з іменем $DEVICE1. Ще раз перевірте значення $DEVICE1 за допомогою echo.  
   Введіть наступні 2 окремі команди, щоб виконати це:
...
#### Щоб призначити адреси IPv6 інтерфейсам `veth`
...
2. Налаштуйте підключення `macvtap2` із такими параметрами:

    IPv4 Method =  manual IPv4 Addresses =  172.16.99.200/24 Gateway = 172.16.99.1 DNS Servers  = 8.8.8.8 and 8.8.4.4 DNS Search domain = example.com
