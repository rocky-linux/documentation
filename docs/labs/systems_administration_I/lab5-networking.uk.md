...
1. Увійшовши у свою систему, перегляньте поточне _ім'я хоста_ за допомогою популярної утиліти `hostname`. Впишіть:
...
3. Використайте утиліту `hostnamectl` для перегляду поточних _імен хостів_. Впишіть:
...
4. Додайте опцію `--static` до команди `hostnamectl`, щоб переглянути статичне ім'я хоста вашого сервера. Впишіть:
...
4. Тепер створіть інтерфейс MACVTAP з назвою `macvtap1`. Новий інтерфейс буде пов'язаний з
   $DEVICE1. Впишіть:
...
3. Зробіть запит до системи, щоб побачити маршрут, який буде використано для досягнення довільного прикладу пункту
   призначення. Наприклад, щоб переглянути деталі маршрутизації для досягнення IP-адреси призначення
   8.8.8.8, type:
...
5. Перевірте зміни в мережевому з'єднанні за допомогою команди `ip`. Впишіть:

    ```bash
    ip -br address show  dev macvtap2
    ```

6. Щоб правильно застосувати нові налаштування та зробити їх новими значеннями середовища виконання, скористайтеся `nmcli`, щоб спочатку вимкнути з'єднання (тобто деактивувати його). Впишіть:
