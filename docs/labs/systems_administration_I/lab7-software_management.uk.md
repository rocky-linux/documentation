...
2. Тепер запустіть команду `rpm` з опцією `--verify`. Впишіть:
   ...
   - **Перший стовпець (S.5....T.)**
     ...
     Використовуйте 9 полів для зберігання дійсної інформації файлу після встановлення пакета RPM. Будь-яке поле чи характеристика, які пройшли певну перевірку/тест, позначається знаком «.».
