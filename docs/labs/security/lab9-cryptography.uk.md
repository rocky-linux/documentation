- - -
Title: Лабораторна робота 9 - Криптографія author: Wale Soyinka contributors:
- - -
...
!!! NOTE "Примітка"
    ...
    У вашому key-ring ще не повинно бути ключів. Але наведена вище команда також допоможе створити середовище за замовчуванням, щоб ви могли виконати нову пару ключів з першого разу.
...
Будь ласка, вкажіть, як довго має бути дійсний ключ.

 0 = термін дії ключа не закінчується

<n> = термін дії ключа закінчується через n днів

<n>w  = термін дії ключа закінчується через n тижнів

<n>m = термін дії ключа закінчується через n місяців

<n>y = термін дії ключа закінчується через n років
...
Вам потрібен ідентифікатор користувача, щоб ідентифікувати ваш ключ; програмне забезпечення створює ідентифікатор користувача

від Real Name, Comment та Email Address в цій формі:
...
2. Щоб придушити дратівливе «попередження» про «незахищену пам’ять», додайте наступний параметр до вашого файлу конфігурації gpg. Впишіть:
...
!!! NOTE "Примітка"
    ...
    Фактичний ідентифікатор ключа - 1D12E484
...
ПЕРЕКОНАЙТЕСЯ, ЧИ ВАШ ПАРТНЕР ВИКОНАВ ВСЕ ПОВНІСТЮ
...
ВИ МОЖЕТЕ ВИКОРИСТОВУВАТИ БУДЬ - КОРИСТУВАЧ «me@serverXY» або КОРИСТУВАЧ «ying@serverPR»

ЯК ВАШ ПАРТНЕР У НАСТУПНІЙ ВПРАВІ.
...
!!! NOTE "Примітка"
    ...
    Будьте дуже обережні під час розшифровки бінарних файлів (наприклад, програм), тому що після успішного розшифрування файлу gpg спробує надіслати вміст файлу на стандартний вивід.
...
автентифікацію, виконання команд і обмін даними. Згідно зі сторінкою довідки sshd, sshd працює наступним чином:

Для протоколу SSH версії 2:

Кожен хост має спеціальний ключ (RSA або DSA), який використовується для ідентифікації хоста. Коли демон запускається, він не генерує ключ сервера (як у протоколі SSH версії 1). Передня безпека забезпечується через ключову угоду Діффі-Хеллмана. Ця угода ключа призводить до спільного ключа сеансу.
...
Протокол версії 2 надає метод автентифікації на основі відкритого ключа користувача (PubkeyAuthentication) або хоста клієнта (HostbasedAuthentication), звичайну автентифікацію за паролем і методи на основі виклику.
...
sshd_config є досить дивним конфігураційним файлом.

На відміну від інших конфігураційних файлів Linux, ви помітите, що коментарі (\#) у файлі sshd\_config позначають значення параметрів за замовчуванням. тобто коментарі представляють уже скомпільовані параметри за замовчуванням.
...
Ваш сервер ssh уже має ключі хостів, які він використовує. Ці ключі були згенеровані під час першого встановлення вашої системи. У цій вправі ви дізнаєтеся, як створити ключі хост-типу для вашого сервера. Але ви не будете використовувати ключі.
...
```
Коли буде запропоновано, введіть пароль користувача ying. Якщо ви отримаєте будь-які попередження, введіть «yes», щоб продовжити.
...
Загальнодоступний ключ зберігається у файлі з тим же ім’ям, що й закритий ключ, але з розширенням «.pub». Немає способу відновити втрачену парольну фразу. Необхідно створити новий ключ, якщо парольну фразу втрачено або забуто.
...
```
 ...
 як користувач ying на serverPR.
...
Згідно зі сторінкою довідки, ssh-agent — це програма для зберігання закритих ключів для автентифікації відкритих ключів (RSA, DSA). Ідея полягає в тому, що ssh-agent запускається на початку X-сеансу або сеансу входу, а всі інші вікна або програми запускаються як клієнти для програми ssh-agent. Використовуючи змінні середовища, агент можна знайти та автоматично використовувати для автентифікації під час входу на інші машини за допомогою ssh.
...
У цій вправі ви дізнаєтесь, як налаштувати агента таким чином, щоб вам не доводилося вводити свою парольну фразу кожного разу, коли ви хочете підключитися до іншої системи за допомогою автентифікації з відкритим ключем.
