...
Демон OpenSSH SSH підтримує лише протокол SSH 2.  Кожен хост має спеціальний ключ, який використовується для ідентифікації хоста.  Щоразу, коли клієнт підключається, демон відповідає своїм публічним ключем хосту.  Клієнт порівнює ключ хоста з власною базою даних, щоб переконатися, що він не змінився.  Передня безпека забезпечується через ключову угоду Діффі-Хеллмана. Ця угода ключа призводить до спільного ключа сеансу.  Решта сеансу шифрується за допомогою симетричного шифру.
...
6. Перевірити стан служби sshd. Впишіть:
    ...
    ...<SNIP>...
    ```
7. Якщо ви вносите будь-які зміни у файл конфігурації sshd, ви можете перезапустити службу sshd, виконавши:
...
8. На serverXY спробуйте віддалено перезапустити службу sshd, що працює на serverPR, від імені користувача `ying`. Впишіть:
   ...
   ```
    ...
    !!! Questions "Питання"
     - Чи міг користувач віддалено переглянути статус служби sshd на serverPR? Чи зміг користувач віддалено перезапустити службу sshd на serverPR? Напишіть коротке пояснення поведінки, яку ви спостерігаєте
...
5. Яка команда скопіювати всі файли в “/home/me/.gnugp/” на serverPR?
...
8. Використовуйте команду `ls -alR`, щоб переглянути перелік вмісту двох попередніх кроків. Впишіть:
    ...
    !!! Question "Питання":
     Надайте коротке пояснення результату команди `ls -alR`? Поясніть, наприклад, чому у вас є дублікати цих файлів .bash_history, .bashrc
...
Загальнодоступний зберігається у файлі з таким же ім’ям, що й закритий ключ, але з доданим до нього розширенням «.pub». Немає простого способу відновити втрачену парольну фразу. Необхідно створити новий ключ, якщо парольну фразу втрачено або забуто.
...
2. Запустіть програму `ssh-keygen`, щоб створити ключ типу «dsa» із довжиною за замовчуванням. Впишіть:
    ...
    Вам двічі буде запропоновано ввести парольну фразу. Введіть правильну парольну фразу, яку досить важко вгадати. Після цього натисніть <kbd>ENTER</kbd>
...
3. cd у ваш каталог `~/.ssh/`. Перелічіть файли у каталозі.
...
Це означає, що ви знаєте пароль відповідного облікового запису на віддаленій стороні для успішного входу.
...
3. Введіть наведені нижче команди точно так, як показано. Вам буде запропоновано ввести пароль ying на serverPR. Впишіть:
    ...
    b. запустіть команду “cd ~/.ssh && кіт - >> авторизовані_ключі && chmod 600 authorized_keys” як користувач, який знаходиться на serverPR.
