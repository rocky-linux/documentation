...
**a)** - **SHA-1 (Secure Hash Algorithm)** -Це криптографічний алгоритм хешування, опублікований урядом Сполучених Штатів. Він створює 160-бітне хеш-значення з рядка довільної довжини. Вважається, що це дуже добре.

**b)**- **MD5 (Message Digest Algorithm 5)** - це криптографічний алгоритм хешування, розроблений у RSA Laboratories. Його можна використовувати для хешування байтового рядка довільної довжини до 128-бітного значення.
...
\*\*a) \*\*-- **Симетричні алгоритми шифрування (secret-key)**
...
**iv.)** Безпека електронної пошти та пов'язані з нею послуги
...
4. Перелічіть ключі, які зараз є у вашому keyring. Впишіть:
   ...
   !!! note
            ...
            У вашій зв'язці ключів ще не повинно бути жодних ключів. Але наведена вище команда також допоможе створити середовище за замовчуванням, яке дозволить вам успішно створити нову пару ключів з першого разу.

       !!! question "Лабораторні завдання:"

        ```
         Знову перелічіть приховані каталоги у вашому домашньому каталозі. Як називається новий доданий каталог?
        ```
...
5. Використовуйте програму gpg для створення нових пар ключів. Впишіть:
   ...
   !!! warning
            ...
            Варіант (1) створить для вас дві пари ключів. Пара ключів DSA буде первинною парою ключів — для створення цифрових підписів, а підпорядкована пара ключів ELGamel — для шифрування даних.
...
7. Створіть ключі, термін дії яких закінчиться через рік. Введіть «1y» у рядку нижче:
   ...
   \++"n"+"m"++> = термін дії ключа закінчується через n місяців
...
9. Створіть ідентифікатор користувача, щоб ідентифікувати свій ключ за допомогою:
   ...
   "Ім'я Прізвище (будь-який коментар) <yourname@serverXY&>"
...
1. Експортуйте свій відкритий ключ у двійковому форматі у файл під назвою «ying-pub.gpg». Впишіть:
   ...
   !!! note
            ...
            Будь ласка, замініть <your_key’s_user_ID> будь-яким рядком, який правильно ідентифікує ваші ключі. У нашій системі-прикладі це значення може бути будь-яким із наведених нижче:
...
4. Щоб скинути налаштування терміналу, введіть: `reset`
...
10. Відкрийте файл, щоб переглянути його вміст за допомогою будь-якого пейджера.

    !!! question

            Чи можете ви прочитати текст, який ви ввели у файл?

!!! warning "Прочитайте, перш ніж продовжити"

    Переконайтеся, що ваш партнер виконав усі «Вправи 1, 2 та 3» вище, перш ніж ви перейдете до вправи 4 нижче.
    
    Якщо у вас немає партнера, вийдіть з облікового запису користувача Ying та увійдіть у систему як користувач «me».
    
    Потім повторіть усі «Вправи 1, 2 та 3» як користувач «me».
    ...
    Ви можете використовувати користувача «me@serverXY» або користувача «ying@serverPR» як свого партнера в наступній вправі.
...
2. Зробіть свій ASCII-броньований файл відкритого ключа (ying-pub.asc) доступним для вашого партнера (використайте
   ...
   !!! note
            ...
            Існує кілька способів зробити це, наприклад, електронною поштою, копіюванням та вставкою, scp, ftp, збереженням на дискеті тощо.
           
            Оберіть для себе найефективніший метод.
...
7. Переглянути відбиток ключа для me@serverXY. Впишіть:
   ...
   <span id="anchor-4"></span>Шифрування та розшифрування файлів
...
3. Зробіть зашифрований файл, який ви створили, доступним для належного власника та попросіть його виконати наведену вище команду, щоб розшифрувати файл. Чи вдалося їм розшифрувати файл.
   ...
   !!! note
            ...
            Будьте дуже обережні під час розшифрування бінарних файлів (наприклад, програм), оскільки після успішного розшифрування файлу gpg спробує вивести вміст файлу на стандартний вивід.
   ...
   !!! Tip "Поради"
        ...
        ```
         Більшість команд і опцій, що використовуються програмою gpg, також мають короткі форми, що зменшує кількість введених користувачем команд у командному рядку. Наприклад,
        
             ```
             gpg --encrypt --recipient me@serverXY encrypt-sec.txt
             ```
             Коротка форма вищезазначеної команди така:
...
Серверна частина включає демон захищеної оболонки (`sshd`). `sshd` — це демон, який прослуховує з'єднання від клієнтів.
...
Більшість систем Linux із коробки вже мають сервер OpenSSH, налаштований і запущений із деякими параметрами за замовчуванням. Файл конфігурації для `sshd` зазвичай знаходиться в `/etc/ssh/` і має назву `sshd_config`.
...
1. Відкрийте конфігураційний файл SSH-сервера будь-яким пейджером і вивчіть його. Впишіть:
   ...
   !!! note
            ...
            `sshd_config` — це досить дивний конфігураційний файл. На відміну від інших конфігураційних файлів Linux, коментарі (#) у файлі `sshd_config` позначають значення опцій за замовчуванням (тобто коментарі представляють вже скомпільовані значення за замовчуванням)

2. Зверніться до сторінки довідки щодо `sshd_config`.

   !!! question

            Що роблять наступні опції?
...
2. Використайте програму `ssh-keygen` для створення ключа хоста з такими характеристиками:
   ...
   !!! question "Питання"
            ...
            Що потрібно зробити, щоб демон sshd використовував щойно згенерований вами ключ хоста?
...
7. Якщо ви внесете будь-які зміни до конфігураційного файлу `sshd`, ви можете перезапустити службу `sshd`, виконавши команду:
...
У цьому розділі розглядаються вправи, що охоплюють клієнтську програму `ssh`.
...
5. Використовуйте `ssh` для віддаленого виконання команди “ls” для рекурсивного перегляду списку файлів у домашньому каталозі користувача serverPR. Впишіть:
...
6. Під час авторизації на сервері serverXY спробуйте віддалено перезавантажити serverPR від імені користувача `ying`. Впишіть:
   ...
   !!! Questions "Питання"
        ...
        ```
         Чи зміг користувач ying віддалено перезавантажити serverPR? Чому ying не може віддалено перезавантажити serverPR?
        ```
...
8. З сервера serverXY спробуйте віддалено перезапустити службу `sshd`, що працює на serverPR, від імені користувача `ying`. Впишіть:
   ...
   !!! Questions "Питання"
        ...
        ```
         - Чи міг користувач ying віддалено переглядати стан служби sshd на serverPR? 
         - Чи зміг користувач ying віддалено перезапустити службу sshd на serverPR?
         - Напишіть коротке пояснення поведінки, яку ви спостерігаєте.
        ```

9. Введіть «exit», щоб вийти з serverPR і повернутися до serverXY.

### `scp` - безпечне копіювання (програма для віддаленого копіювання файлів)

`scp` копіює файли між хостами в мережі. Він використовує SSH для передачі даних, використовує ту саму автентифікацію та забезпечує ту саму безпеку, що й `ssh`.
...
1. Переконайтеся, що ви все ще ввійшли в систему як користувач `me` на serverXY.

2. Створіть каталог під назвою `myimport` у вашому домашньому каталозі та перейдіть до нього за допомогою cd.

3. Використайте `scp` для копіювання всіх файлів з каталогу “/home/me/myexport/” на віддаленому сервері serverPR.
   (крапка «.» у кінці команди важлива). Впишіть:
   ...
   !!! question "Питання"
            ...
            Напишіть коротке пояснення, чому попередня команда не виконала свою роботу?

4. Знову виконайте попередню команду, але цього разу додайте рекурсивний параметр до `scp`. Впишіть:
...
7. Знову виконайте невеликий варіант попередньої команди, щоб скопіювати домашній каталог ying на serverPR.  Впишіть:
   ...
   !!! Questions "Питання"
        ...
        ```
         Яка невелика, але суттєва різниця між варіантами двох попередніх команд? І який результат кожної команди?
...
8. Використайте команду `ls -alR`, щоб переглянути список вмісту двох попередніх кроків. Впишіть:
   ...
   !!! question "Питання"
            ...
            Надайте коротке пояснення результату виконання команди `ls -alR`. Поясніть, наприклад, чому у вас, здається, є дублікати цих файлів .bash_history, .bashrc ...

## Завдання 7

### Створення відкритих і закритих ключів користувача для SSH

Кожному користувачеві, який хоче використовувати SSH з автентифікацією RSA або DSA, потрібна пара відкритого та закритого ключів. Для створення цих ключів можна використовувати програму `ssh-keygen` (так само, як вона використовувалася раніше, коли ви створювали нові ключі хоста для вашої системи)

!!! TIP "Підказка"

    ```
    Одна з головних відмінностей між ключами хоста та користувача полягає в тому, що наполегливо рекомендується захищати ключі користувача за допомогою парольної фрази. Парольна фраза — це пароль, який використовується для шифрування закритого ключа [звичайного тексту].
    ```
...
2. Запустіть програму `ssh-keygen`, щоб створити ключ типу «dsa» з довжиною за замовчуванням. Впишіть:
   ...
   Після успішного завершення ви побачите повідомлення про те, що ваші ідентифікаційні дані та відкриті ключі збережено в каталозі `/home/ying/.ssh/`.

3. перейдіть до вашого каталогу `~/.ssh/` за допомогою команди cd. Список файлів у каталозі.

4. Яка команда `ssh-keygen` для перегляду відбитка ваших ключів?

5. Використайте команду cat для перегляду вмісту вашого файлу відкритого ключа (наприклад, `~/.ssh/id_dsa.pub`).
...
1. Увійдіть у свою локальну систему як користувач _ying_.

2. cd у ваш каталог «~/.ssh».

3. Введіть наведені нижче команди точно так, як показано. Вам буде запропоновано ввести пароль ying на serverPR.
   Впишіть:
   ...
   a. отримати вміст вашого файлу відкритого ключа dsa та надіслати ( | ) вивід до `ssh ying@serverPR`

   b. виконайте команду “cd ~/.ssh && cat - >> authorized_keys && chmod 600 authorized_keys” від імені користувача serverPR.

   !!! Note "Примітка"

        ```
         Мета попередньої складної команди — скопіювати та додати вміст вашого файлу відкритого ключа до «/home/ying/.ssh/authorized_keys» на serverPR та надати йому правильні дозволи.
        ```

   !!! Tip "Порада"

        ```
         Ви можете скористатися утилітою `ssh-copy-id` для легкого та більш витонченого налаштування автентифікації за допомогою відкритого/приватного ключа між системами. `ssh-copy-id` – це скрипт, який використовує `ssh` для входу на віддалену машину (ймовірно, спочатку з використанням пароля для входу). 
         Він складає список одного або кількох відбитків пальців (як описано нижче) та намагається увійти за допомогою кожного ключа, щоб перевірити, чи якийсь із них вже встановлений. Потім він складає список тих, яким не вдалося увійти, та за допомогою `ssh` дозволяє вхід за допомогою цих ключів на віддаленій системі. За замовчуванням він додає ключі, додаючи їх до ~/.ssh/authorized_keys віддаленого користувача (створюючи файл та каталог, якщо необхідно).
        ```
...
1. Переконайтеся, що ви ввійшли у свою локальну систему як користувач _ying_.
...
3. Запустіть програму `ssh-add`, щоб отримати список відбитків усіх [публічних/приватних] ідентифікаційних даних, які наразі представлені агентом. Впишіть:
...
4. Використайте програму `ssh-add` без будь-яких опцій, щоб додати свої ключі до агента, якого ви запустили вище. Впишіть:
...
5. Тепер знову запустіть команду `ssh-add`, щоб переглянути список відомих ідентифікаторів відбитків пальців. Впишіть:
...
6. Тепер, коли користувач _ying_, спробуйте віддалено підключитися до serverPR та виконати просту тестову команду.
...
7. Якщо ви закінчили і більше не потребуєте послуг `ssh-agent`, або просто хочете повернутися до автентифікації на основі ключів, ви можете видалити всі [приватні/публічні] ідентифікатори з агента. Впишіть:
