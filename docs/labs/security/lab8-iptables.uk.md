...
> Я завжди використовую iptables на своїх пристроях Linux. Подивіться – навіть якщо ви не хочете тримати хакерів подалі, ви можете заблокувати рекламні сайти, такі як doubleclick.com та інші. Або, можливо, ви хочете зробити трохи більше журналу? …Iptables вас врятує!
...
Утиліта командного рядка iptables надає інтерфейсні (користувацькі) інструменти для керування цією підсистемою. Він використовується для налаштування, підтримки та перевірки таблиць правил фільтрації IP-пакетів у ядрі. Можна визначити кілька різних таблиць.
...
* filter:      Це основна таблиця та таблиця за замовчуванням (якщо параметр -t не передано).  Він містить вбудовані ланцюжки INPUT (для пакетів, що надходять у сам ящик), FORWARD (для пакетів, що направляються через ящик), і OUTPUT (для локально згенерованих пакетів).

* nat:       До цієї таблиці звертаються, коли зустрічається пакет, який створює нове з’єднання.  Вона складається з наступних трьох вбудованих ланцюжків: ** PREROUTING (для зміни пакетів, щойно вони надходять) ** OUTPUT (для зміни локально згенерованих пакетів перед маршрутизацією) ** POSTROUTING (для зміни пакетів, коли вони збираються вийти)

* mangle:    Ця таблиця використовується для спеціалізованої зміни пакетів.  Вона має наступні 5 вбудованих ланцюгів: PREROUTING (для зміни вхідних пакетів перед маршрутизацією) та  
  OUTPUT  (для зміни локально згенерованих пакетів перед маршрутизацією), INPUT (для пакетів, що надходять у саму коробку),     
  FORWARD (для зміни пакетів, що направляються через ящик), та  
...
1. Увійшовши як суперкористувач, перелічіть усі правила в таблиці фільтрів. Впишіть:
...
1. Перш ніж почати, переконайтеся, що ви можете пінгувати вашу партнерську систему, і що ваша партнерська система може

надто успішно пінгувати вас. Впишіть:
...
6. Очистіть усі правила та повторіть команду ping з обох систем. Успіх чи невдача?
...
Ви прочитали б наведену вище команду простою мовою так:

*«Додайте правило в ланцюжок INPUT у таблиці фільтрів. Нехай це правило видаляє всі пакети типу icmp і має адресу джерела 172.16.0.44”*
...
9. Замість того, щоб скидати всі правила у ваших таблицях. Видаліть лише правило, яке ви створили вище. Для цього

потрібно знати номер правила. Щоб дізнатися тип номера правила:
...
2. Попросіть свого партнера спробувати увійти на ваш ftp-сервер як анонімний користувач. Переконайтеся, що ваш партнер

здатен успішно увійти з serverPR - зробіть це *перед* переходом до наступного кроку.



3.  Поки ваш партнер ще ввійшов у систему, створіть правило, щоб вимкнути весь трафік типу ftp, що надходить з

serverPR. Впишіть:

[root@serverXY root]# iptables -A INPUT -i  eth0 -s 172.16.0.z  -p tcp  --dport 21 -j DROP*

Якщо говорити простою мовою, наведене вище правило/команда перекладається так:

*Додайте правило до ланцюжка INPUT у таблиці фільтрів. Нехай це правило ВИДАЛИТЬ усі пакети з адресою джерела 172.16.0.z і призначені для порту 21 у нашій локальній системі.*

4. Щойно ви виконаєте наведену вище команду, стек netfilter негайно введе її в дію. Щоб побачити

це, попросіть свого партнера спробувати будь-які ftp-команди, залишаючись на вашому ftp-сервері - напр. ls

    Успіх чи невдача?

Якщо це не вдасться, попросіть свого партнера спробувати від’єднатися та знову ввійти з нуля.

    Успіх чи невдача?

5. Попросіть іншу особу, яка НЕ є вашим партнером, спробувати анонімно увійти на ваш ftp-сервер.

Ви також можете попросити когось із hq.example.org спробувати підключитися до вашого ftp-сайту.

    Успіх чи невдача?

6.  Увімкніть і запустіть веб-сервер на serverXY.

7.  Переконайтеся, що інші люди можуть відвідувати ваш веб-сайт за допомогою браузера. Створіть правило для блокування http-трафіку

з hq.example.org на вашу локальну машину.
...
Якщо бути педантичним, то маскування IP-адреси та NAT-інг насправді є різними звірами, які зазвичай використовуються для досягнення різних цілей. Ми не будемо надто зупинятися на конкретних відмінностях у наступних вправах.
...
2. Призначте всім інтерфейсам відповідну IP-адресу, маску мережі та параметри шлюзу.

3.  Очистіть усі правила iptables, які ви зараз завантажили.

!!!NOTE "Примітка":
Очищення таблиць не завжди є необхідним або обов'язковим. Можливо, ви помітили, що на початку деяких виконаних вправ ми вказали, що ви очищаєте існуючі таблиці. Це робиться для того, щоб ви почали з чистого аркуша та щоб десь у ваших таблицях не ховалися помилкові правила, які можуть спричинити неправильну роботу. Зазвичай у вас можуть бути завантажені сотні правил одночасно, які виконують різні функції.

4.  Попросіть свого партнера в serverPR спробувати пінгувати 172.16.0.100 (hq.example.org), це має завершитися помилкою, тому що

тепер ви працюєте як шлюз за замовчуванням serverPR і ви *ще не* ввімкнули маршрутизацію на своїй
...
```
6.  Тепер знову повторіть крок 4. Успіх чи невдача?
...
```
8.  Щоб зробити вказану вище зміну в ядрі постійним між перезавантаженнями, створіть запис нижче у своєму файлі
...
До цього часу всі правила iptables і ланцюжки, які ви створювали, були ефемерними або непостійними. Це означає, що якщо вам доведеться перезавантажити систему в будь-який момент, усі внесені вами правила та зміни буде втрачено.

Щоб запобігти цьому, вам потрібен механізм для запису або збереження тимчасових правил iptables під час виконання в системі, щоб вони завжди були доступними під час перезавантаження системи.
...
!!!TIP "Підказка"
Ви обмежені тим, що ви можете з iptables, лише вашою уявою. Ми ледь подряпали поверхню у цій лабораторній роботі. Сподіваємось, ми достатньо подряпали поверхню, щоб дозволити вашій уяві розквітнути.
...
1. Який параметр потрібен, щоб отримати більш детальну версію цієї команди *iptables -L -t nat*?
...
6a. Створіть правило iptables, щоб заблокувати http-трафік від hq.example.org до вашої локальної машини.

6b. Який добре відомий порт прослуховують веб-сервери?

6c.  Запишіть повну команду для досягнення цього?

6d. Перетворіть або перекладіть команду, яку ви записали вище, на її звичайний еквівалент?
