...
This document will record a summary of all QA and Testing results for the Rocky Linux 8.6 release. It is only a record of success and/or failure. Solution discussion should take place elsewhere.

## SOP

- Please include PASS, FAIL, NOTABUG, INVESTIGATE, or UPSTREAM as appropriate in all entries.
- Please only provide a brief summary. Details should go to Rocky Pastebin; links here are OK.
- Especially for a negative result, please leave your MM @handle so we can talk to you to get a resolution.
- If the item you are reporting is related to a QA: Testcase, please mention it. If it should be a QA: Testcase, even if it's not a current requirement, suggest a title and create an issue in the wiki repository so we can add it.

## INVESTIGATE

- errors in all tests in openQA - INVESTIGATE - see openQA section below.
- INVESTIGATE whether the kdump issue affects QCows with encrypted partitions in createhdds. These are pre-reqs for openQA multi-machine tests. NOTE: This is addressed below in section - @tcooper
...
- QA:Testcase Media File Conflicts - EXPECTED(per @label) for Rocky-8.6-x86_64-dvd1.iso (4 file conflicts found and 13 package conflicts found, these appear to be the same as the 8.5 conflict between mariadb and mysql packages/files, full results - <https://rpa.st/raw/ZWPQ>) - @tcooper
...
- the dvd1 iso of aarch64 doesn't show a workstation base environment - it doesn't have a workstation environment - PASS - @lumarel
...
- QA: Testcase boot/install minimal x86_64 over DVD/Bluray (burned with Fedora Media Writer) on G752 ASUS laptop - PASS - @atomicturtle
...
Test method: Used the packer to build a VM.  Booted VM.  Verified root login.  Shutdown VM.
