---
title: Testing Team & QA
author: Trevor Cooper
contributors: Bob Robison
tested_with:
tags:
  - Softwaretest
revision_date: 2026-06-05
render_macros: true
---

## Links

- Rocky Linux Mattermost: [~Testing](https://chat.rockylinux.org/rocky-linux/channels/testing)
- Rocky Linux openQA: [https://openqa.rockylinux.org](https://openqa.rockylinux.org)

## Verantwortlichkeiten

Das Testing Team ist für das Testen und die Qualitätssicherung von Rocky Linux zuständig.

## Besprechungen / Kommunikation

- Team-Meeting wöchentlich:
  [Google Meet](http://meet.google.com/kqs-rxme-juw),
  [Meeting-Berichte](https://git.resf.org/testing/meetings)
  - Jeden Donnerstag, abwechselnd 23:00 Uhr UTC und 14:00 Uhr UTC – siehe [RESF Public Calendar](https://calendar.google.com/calendar/u/0/embed?src=c_2e1oqh6t0i6sqhja5nu9lq8lgo@group.calendar.google.com)

## Mitglieder

Eine Liste unserer Mitglieder finden Sie auf der Seite [Mitglieder](members.md).

{% include "teams/testing/content_bottom.md" %}
