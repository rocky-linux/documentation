...
Ми не збираємося розглядати «чому» ці зміни в цій статті; ми просто обговоримо, що має статися, якщо це технічний контроль.  Якщо ми нічого не можемо змінити, як у випадку з операційним елементом керування, поле **Виправити:** буде встановлено на значення «ні». Хороша новина в багатьох із цих випадків, це вже за замовчуванням у Rocky Linux 8, тому вам взагалі нічого не потрібно змінювати.
...
**Severity:** Cat II Medium  
**Type:** Operational  
**Fix:** Немає. Щоб захистити цілісність даних, що записуються у файли журналів, переконайтеся, що лише члени групи Аудитори, Адміністратори та користувач, призначений для запуску програмного забезпечення веб-сервера, мають дозволи на читання файлів журналів.
...
**(V-214274)** Файли htpasswd веб-сервера Apache (якщо є) мають відображати належне право власності та дозволи.

**Severity:** Cat II Medium  
**Type:** Operational  
**Fix:** Немає. Переконайтеся, що файл "htpasswd" належить обліковому запису SA або веб-менеджера. Переконайтеся, що для дозволів встановлено значення «550».
...
**Severity:** Cat II Medium  
**Type:** Operational  
**Fix:** немає. Зверніться до адміністратора SIEM, щоб визначити, чи налаштовано SIEM на виділення достатнього обсягу сховища записів журналів для задоволення вимог веб-сервера Apache до ведення журналу.
