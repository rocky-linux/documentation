...
При використанні опції `-n`, оболонка автоматично перевіряє введені дані після зазначеної кількості символів. Користувачеві не потрібно натискати клавішу ++enter++.

```bash
read -n5 name
```

Команда `read` дозволяє перервати виконання скрипта, поки користувач вводить інформацію. Введені користувачем дані розбиваються на слова, призначені одній або кільком попередньо визначеним змінним. Слова — це рядки символів, розділені роздільником полів.
...
Поділ слів визначається символом роздільника полів.
Цей роздільник зберігається в системній змінній `IFS` (**Внутрішній роздільник полів**).
...
Нижче наведено приклад використання команди `tr`. Якщо ви використаєте `grep` для повернення запису файлу `passwd` root-користувача, ви отримаєте ось це:
...
Незважаючи на те, що цей метод дуже зручний для користувача, він має свої обмеження, якщо сценарій планується виконувати вночі.
Щоб подолати цю проблему, можна ввести бажану інформацію за допомогою аргументів.
...
Аргументи заповнюються під час введення команди сценарію.
Вони розділені пробілом.
