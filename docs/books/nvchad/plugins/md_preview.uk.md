...
Зокрема, документація, написана для Rocky Linux, конвертується в `HTML` за допомогою програми _python_. Програма конвертує документи, написані в _markdown_, на статичні HTML-сторінки.
...
Щоб інтегрувати цю функціональність у ваш редактор, на цій сторінці будуть проілюстровані два плагіни, доступні для цієї мети, [toppair/peek.nvim](https://github.com/toppair/peek.nvim) та [markdown-preview.nvim](https://github.com/iamcco/markdown-preview.nvim). Обидва підтримують _стиль github_, вибір браузера для попереднього перегляду та синхронізовану прокрутку з редактором.

### Peek.nvim

[Peek](https://github.com/toppair/peek.nvim) використовує середовище виконання [Deno](https://docs.deno.com/runtime/manual/) на JavaScript, TypeScript та WebAssembly з налаштуваннями за замовчуванням та безпечними налаштуваннями для його роботи. За замовчуванням Deno не дозволяє доступ до файлів, мережі чи середовища, якщо це явно не ввімкнено.

Для встановлення мовного сервера в конфігурації редактора використовується плагін _mason.nvim_, який надає команду `:MasonInstall`, команду, яка дозволяє автоматично включати та налаштовувати _Deno_.
...
Після збереження файлу ви можете виконати його встановлення, відкривши інтерфейс менеджера плагінів за допомогою команди `:Lazy`. Менеджер плагінів вже розпізнає його автоматично та дозволить вам встановити його, ввівши ++"I"++.

Однак, щоб отримати повну функціональність, вам потрібно закрити NvChad (_nvim_) та знову відкрити його. Це дозволяє редактору завантажувати елементи **Peek** до конфігурації.
...
Як і в попередньому плагіні, вам потрібно буде закрити редактор і знову відкрити його, щоб дати NvChad можливість завантажити нову конфігурацію. Знову ж таки, ви можете передати плагіну деякі користувацькі опції, описані у [спеціальному розділі](https://github.com/iamcco/markdown-preview.nvim#markdownpreview-config) репозиторію проекту.
...
У конфігурації також передбачено команди `:MarkdownPreview` та `:MarkdownPreviewStop` для відкриття та закриття попереднього перегляду відповідно. Для швидшого доступу до команд ви можете зіставити їх з файлом **mapping.lua** наступним чином:
