...
Завдяки використанню ==**L**anguage **S**server **P**rotocols== та ==linters==, ви можете налаштувати середовище, яке буде налаштоване саме для вас. Найкраще те, що після налаштування середовища його можна швидко оновлювати, коли з'являються нові зміни, за допомогою [lazy.nvim](https://github.com/folke/lazy.nvim) та [Mason](https://github.com/williamboman/mason.nvim), обидва з яких описані тут.

Оскільки Neovim є форком [Vim](https://www.vim.org/), загальний інтерфейс буде знайомий користувачам _vim_. Якщо ви не користувач _vim_, то за допомогою цієї книги ви швидко ознайомитеся з синтаксисом команд. Тут викладено багато інформації, але за нею легко стежити, і як тільки ви ознайомитеся з усіма матеріалами, ви знатимете достатньо, щоб створити власне IDE для _ваших_ потреб за допомогою цих інструментів.

Автор не мав наміру розбивати цю книгу на розділи. Причина в тому, що це передбачає порядок, якого потрібно дотримуватися, і, здебільшого, це не обов’язково. Вам _потрібно_ почати з цієї сторінки, прочитати та виконати дії з розділів «Додаткове програмне забезпечення», «Встановлення Neovim» та «Встановлення NvChad», але звідти ви можете вибрати, як ви хочете продовжити.
...
- :material-invert-colors: **Привабливий інтерфейс.** Незважаючи на те, що це програма _cli_, інтерфейс виглядає сучасно та красиво графічно, а всі компоненти підходять інтерфейс ідеально.
- :material-file-settings-outline: **Надзвичайно налаштовуваний.** Завдяки модульності, що походить від базової програми (NeoVim), редактор можна ідеально адаптувати до власних потреб. Однак пам’ятайте, що коли ми говоримо про налаштування, ми маємо на увазі функціональність, а не зовнішній вигляд інтерфейсу.
- :material-update: **Механізм автоматичного оновлення.** Редактор постачається з механізмом (через використання _git_), який дозволяє оновлення за допомогою проста команда `:NvChadUpdate`.
- :material-language-lua: **На базі Lua.** Конфігурація NvChad повністю написана мовою _lua_, що дає змогу легко інтегрувати її в конфігурацію Neovim за допомогою всього потенціалу редактора, на якому він заснований.
...
[Tree-sitter](https://tree-sitter.github.io/tree-sitter/) складається з двох компонентів: генератора парсерів === та бібліотеки інкрементального парсингу ===. Він може побудувати синтаксичне дерево вихідного файлу та ефективно оновлювати його з кожною зміною.
...
??? note    s "Взаємодоповнюваність LSP і tree-sitter."
