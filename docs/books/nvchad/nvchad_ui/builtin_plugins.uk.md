...
У папці `plugins` знаходяться файли _init.lua_ та _ui.lua_. Перший пов’язаний з налаштуванням плагінів, які пропонують додаткову функціональність редактору (_telescope_, _gitsigns_, _tree-sitter_ тощо), тоді як другий встановлює зовнішній вигляд редактора (кольори, значки, файловий менеджер тощо).
...
Розробники NvChad провели величезну роботу, яку слід відзначити. Вони створили інтегроване середовище серед усіх плагінів, що робить інтерфейс користувача чистим і професійним. Крім того, плагіни, які працюють _під капотом_, дозволяють розширити редагування та використовувати інші функції.
