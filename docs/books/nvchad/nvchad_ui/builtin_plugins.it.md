...
!!! note "Convenzioni di denominazione dei plugin"

    Questo capitolo identificherà il plugin utilizzando il formato `user_github/plugin_name`. Questo per evitare possibili errori con plugin dal nome simile e per introdurre il formato utilizzato per l'inserimento dei plugin sia da NvChad che dalla configurazione `custom`.
...
Nella cartella `plugins` sono presenti i file _init.lua_ e _ui.lua_. Il primo si occupa della configurazione dei plugin che offrono funzionalità aggiuntive all'editor (_telescope_, _gitsigns_, _tree-sitter_, ecc.), mentre il secondo imposta l'aspetto dell'editor (colori, icone, file manager, ecc.).
...
Gli sviluppatori di NvChad hanno svolto un lavoro enorme che va riconosciuto. Hanno creato un ambiente integrato tra tutti i plugins che rende l'interfaccia utente pulita e professionale. Inoltre, i plugin che lavorano _sotto il cofano_ consentono di migliorare l'editing e le altre funzioni.
