...
:heavy_check_mark: встановлювати та керувати ролями.
...
Це лише один приклад того, що можна зробити, використовуючи ролі, надані спільнотою. Завітайте на [galaxy.ansible.com](https://galaxy.ansible.com/), щоб знайти вакансії, які можуть бути вам корисними!
...
Ролі дозволяють відмовитися від необхідності включати файли. Немає потреби вказувати шляхи до файлів або директиви `include` у плейбуках. Вам просто потрібно вказати завдання, і Ansible подбає про включення.
...
Ми створимо користувача `rockstar` на всіх наших серверах. Оскільки ми не хочемо, щоб цього користувача було перевизначено, давайте визначимо його у `vars/main.yml`:
...
Створіть список пакетів для встановлення за замовчуванням на ваших серверах і порожній список пакетів для видалення. Відредагуйте файли `defaults/main.yml` та додайте ці два списки:
...
Очевидно, що немає обмежень щодо того, наскільки ви можете покращити свою роль. Уявіть, що для одного з ваших серверів вам потрібен пакет, який є у списку тих, які потрібно видалити. Тоді, наприклад, ви можете створити новий список, який можна перевизначити, а потім видалити зі списку пакетів для видалення ті, що знаходяться у списку певних пакетів для встановлення, використовуючи фільтр jinja `difference()`.
