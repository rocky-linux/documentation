...
- **Керуюча машина**: машина, на якій встановлено Ansible. Оскільки Ansible працює без агентів, на керованих серверах не розгортається жодне програмне забезпечення.
...
- **Модуль**: модуль абстрагує завдання. Ansible надає багато модулів.
- **Playbooks**: простий файл у форматі yaml із визначенням цільових серверів і завдань, які потрібно виконати.
- **Роль**: роль дозволяє вам упорядковувати плейбуки та всі інші необхідні файли (шаблони, скрипти тощо) для полегшення обміну та повторного використання коду.
...
Зверніть увагу, що ansible постачається зі своєю власною версією python, відмінною від системної версії Python (тут 3.11.2 проти 3.6.8). Вам потрібно буде врахувати це під час встановлення модулів Python, необхідних для вашої інсталяції, за допомогою pip (наприклад, `pip3.11 install PyVMomi`).
...
Конфігураційний файл буде створено автоматично, якщо Ansible було встановлено через пакет RPM. У разі встановлення `pip` цей файл не існує. Нам доведеться створити його вручну завдяки команді `ansible-config`:
...
Перейдіть до файлу інвентаризації за замовчуванням, який знаходиться в `/etc/ansible/hosts`. Деякі приклади наведено та прокоментовано:
...
Групи можна використовувати в інших групах. У цьому випадку необхідно вказати, що батьківська група складається з підгруп з атрибутом `:children`, ось так:
...
Як на керуючій машині, так і на клієнтських комп'ютерах ми створимо користувача `ansible`, призначеного для операцій, що виконуються Ansible. Цьому користувачеві доведеться використовувати права sudo, тому його потрібно буде додати до групи `wheel`.
...
Playbooks Ansible описують політику, яка буде застосована до віддалених систем, щоб примусово їх конфігурувати. Плейбуки пишуться у легкозрозумілому текстовому форматі, який об'єднує набір завдань: формат `yaml`.
...
Для кращої читабельності рекомендовано писати ваші п’єси в повному форматі yaml. У попередньому прикладі аргументи наведено в тому ж рядку, що й модуль, значення аргументу йде після його імені та розділяється символом `=`. Подивіться ту саму playbook на повному yaml:
...
Примітка щодо колекцій: Ansible тепер надає модулі у формі колекцій.
Деякі модулі надаються за замовчуванням у колекції `ansible.builtin`, інші потрібно встановити вручну через:
