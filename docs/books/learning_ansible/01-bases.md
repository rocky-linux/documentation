...
:heavy_check_mark: Create Ansible playbooks;   
...
To offer a graphical interface to your daily use of Ansible, you can install some tools like Ansible Tower (RedHat) which is not free, its opensource counterpart Awx, or other projects like Jenkins and the excellent Rundeck.
...
Ansible is available in the _EPEL_ repository but comes as version 2.9.21, which is quite old now. You should use the `python3-pip` installation, which comes after this, to install the latest version. Regardless, we _will_ need the _EPEL_ repository so even if you are going for the newer version, install the _EPEL_ first:
...
!!! Note
    ...
    `python3-argcomplete` will help you in completing an `ansible` command.
...
* The client machine management inventory file `hosts` where the clients, and groups of clients reside.

As we have installed Ansible with pip, these files do not exist. We will have to create them by hand.
...
As you can see, the file provided as an example, uses the INI format,  which is well known to system administrators. Please note that you can choose another file format (like yaml for example), but for the first tests, the INI format is well adapted to the examples in this lesson.
...
As you may have noticed, the groups are declared in square brackets, followed by the elements belonging to the groups. You can create, for example, a `rocky8` group by inserting the following block into this file:
...
We won't go any further for the moment on the subject of inventory, but if you're interested, consider checking out more [here.](https://docs.ansible.com/ansible/latest/user_guide/intro_inventory.html)
...
!!! Warning
    Since we have not yet configured authentication on our 2 test servers, not all of the following examples will work. They are given as examples to facilitate understanding, and will be fully functional after the following chapters.
...
!!! Note
    As in this example, it is sometimes simpler to separate the declaration of managed devices into several files (by cloud project for example), and provide Ansible with the path to these files, than to maintain a long inventory file.
...
On the management machine and clients, we will create an `ansible` user dedicated to the operations performed by Ansible. This user will have to use sudo rights, so it will have to be added to the `wheel` group.
...
We will need to comment out the default option, and uncomment the "NOPASSWD" option, which will look like this:
...
Future commands on the management server should be done with this new user:
...
!!! Abstract
    You can now test all the commands noted previously in this chapter.
...
Add the comment back in to the following line from the `[defaults]` section in the `/etc/ansible/ansible.cfg` configuration file to prevent password authentication:
...
To help you in discovering Ansible and to get used to searching the documentation, here are some exercises you can do before going on:
...
Now that we have seen how to configure a remote server with Ansible on the command line, we will be able to introduce the notion of a playbook. This is another way to use Ansible, which is not much more complex, but which will make it easier to reuse your code.

## Playbooks

Ansible playbooks describe a policy to be applied to remote systems, to force their configuration. Playbooks are written in an easily understandable text format that groups together a set of tasks: the `yaml` format.
...
* <1> The targetted group or the targetted server must exist in the inventory
...
!!! Tip
    dnf is one of the modules that allows you to give it a list as argument.

Note about collections: Ansible nowadays provides modules in the form of collections.
...
```
Where "[collectionname]" is the actual collection you are installing. (Note that the square brackets here are to set "collectionname" apart from the rest of the command and should not be included in the actual command.)

The previous example should be written as this:
...
```
$ ansible-playbook --syntaxe-check play.yml
```
