...
Часто доводиться об’єднувати різні елементи в один рядок.
Потім ми можемо вказати символ або рядок для вставки між кожним елементом.
...
- `{{ ip_list | ansible.netcommon.ipv4 }}`: відображає лише v4 IPs. Не зупиняючись на цьому, якщо вам потрібно, є багато фільтрів, присвячених мережі.
