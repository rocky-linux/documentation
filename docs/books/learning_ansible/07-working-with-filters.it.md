...
## Unisce gli elementi di una lista
...
I filtri `dict2items` and `itemstodict`, un po 'più complesso da implementare, sono frequentemente utilizzati, soprattutto nei cicli.
...
È possibile unire o filtrare i dati da uno o più elenchi:
...
Se la tua lista contiene valori non univoci, è anche possibile filtrarli con il filtro `univoco`.
...
## Alcuni altri filtri
  ...
  * `{{ ip_list | ansible.netcommon.ipv4 }}`: only displays v4 IPs. Senza soffermarsi su questo, se necessario, ci sono molti filtri dedicati alla rete.
