...
Таким чином, процес створення snapshot сервера точно схожий на робочий сервер. Щоб повністю емулювати налаштування нашого робочого сервера, виконайте всі **розділи 1-4** ще раз на сервері знімків, а після завершення поверніться до цього місця.
...
У нашій лабораторії ми не маємо такої розкоші. Можливо, у вас такий самий сценарій. З цієї причини ви збираєтеся додати IP-адреси та імена серверів до файлу /etc/hosts на основному сервері та снепшот сервері. Вам потрібно буде зробити це від імені користувача root (або _sudo_).
...
Далі вам потрібно дозволити весь трафік між двома серверами. Для цього вам потрібно змінити правила `firewalld`. Спочатку на сервері lxd-primary додайте цей рядок:
...
Вам потрібно буде створити це для lxd-snapshot. Поверніться до [Розділу 6] (06-profiles.md) та створіть профіль "macvlan" на lxd-snapshot, якщо потрібно. Якщо ваші два сервери мають однакові назви батьківського інтерфейсу (наприклад, "enp3s0"), ви можете скопіювати профіль "macvlan" до lxd-snapshot без його повторного створення:
...
Через короткий час копіювання буде завершено. Хочете дізнатися напевно? Створіть `lxc list` на сервері lxd-snapshot. Що має повернути наступне:
...
Зробіть це для кожного знімка на сервері lxd-snapshot. "0" у команді гарантує, що `boot.autostart` вимкнено.

## Автоматизація snapshot процесу

Чудово, що ви можете створювати знімки, коли вам це потрібно, а іноді вам _потрібно_ створювати знімок вручну. Ви навіть можете вручну скопіювати його в lxd-snapshot. Але в усіх інших випадках, особливо для багатьох контейнерів, що працюють на вашому сервері lxd-primary, **останнє**, що вам потрібно робити, це витрачати день на видалення знімків на сервері знімків, створення нових знімків та їх надсилання на сервер знімків. Для основної частини ваших операцій ви захочете автоматизувати процес.
