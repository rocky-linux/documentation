...
У дистрибутивах і клонах RHEL Network Manager постійно змінювався. Через це спосіб роботи профілю `macvlan` не працює (принаймні, порівняно з іншими дистрибутивами) та вимагає трохи додаткової роботи для призначення IP-адрес з DHCP або статично.
...
Щоб детальніше продемонструвати проблему, ви повинні запустити `dhclient` на контейнері Rocky Linux 9.0. Це покаже нам, що профіль macvlan _є_ фактично застосованим:
...
Налаштування статичної IP-адреси трохи відрізняється, але зовсім не складно. Ви повинні змінити файл `.yaml, пов’язаний із підключенням контейнера (`10-lxc.yaml\`). Для цього статичного IP ви будете використовувати 192.168.1.201:
