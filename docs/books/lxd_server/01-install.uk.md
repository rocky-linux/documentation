...
## Встановіть `snapd`, `dkms`, `vim` та `kernel-devel`

Для Rocky Linux LXD потрібно інсталювати з оснастки. З цієї причини вам потрібно встановити `snapd` (та кілька інших корисних програм) за допомогою:
...
Перший файл, який ви повинні змінити, це файл `limits.conf`. Цей файл самодокументований, тому подивіться пояснення у файлі щодо того, що він робить. Перегляньте пояснення в коментарях до файлу, щоб зрозуміти, що робить цей файл. Щоб внести зміни, введіть:
...
Щоб внести ці зміни до ядра, вам потрібно створити файл під назвою `90-lxd-override.conf` у `/etc/sysctl.d`. Щоб зробити цей тип:

```bash
vi /etc/sysctl.d/90-lxd-override.conf
```

!!! warning "RL 9 та MAX значення `net.core.bpf_jit_limit`"
