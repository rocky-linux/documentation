...
Існує велика кількість варіантів налаштування контейнера після його встановлення. Однак, перш ніж розглянемо їх, давайте розглянемо команду `info` для контейнера. У цьому прикладі ви будете використовувати контейнер ubuntu-test:
...
Це покаже вам усі параметри для налаштування контейнера. Якщо у вас є запитання щодо того, що робить один із параметрів конфігурації, перейдіть до [офіційної документації для LXD](https://documentation.ubuntu.com/lxd/en/latest/config-options/) та виконайте пошук параметра конфігурації або знайдіть у Google весь рядок, наприклад, `lxc config set limits.memory`, та перегляньте результати пошуку.
