...
* `/var/log/btmp` - Records the log of login errors. To view this binary file, use the `lastb` command.
* `/var/log/cron` - Records the logs related to the system's scheduled tasks. The file content is plain text.
...
3. obsolete legacy (legacy) - This format has been deprecated. Please do not continue to use it.
...
`*` represents all facilities. You can also use `,` and `;` to combine the facilities in a single-line configuration. `,` means logical or; `;` represents the rule delimiter.
...
* `/var/log/secure` - Saves the log to a local file
* `@192.168.100.20:22` - Remote machine
* `:omusrmsg:root,frank,jack` - Specifies the online user's list. `*` represents all users. "omusrmsg" refers to "output module for user messages".
* `/dev/tty12` - Specific terminal devices
* `-` - Disables buffering mechanism when writing to file

### Log rotation

**Log rotation** - Log rotation solves the problems of storage occupation and performance degradation caused by the continuous growth of log files. The specific functions are:
...
| `create <MODE> <OWNER> <GROUP>` | After log rotation, define the owner, group, and permissions of the new log file                                 |
| `mail <E-MAIL ADDRESS>`         | After log rotation, it sends the output content by email to the specified email address                           |
| `missingok`                     | If the log file does not exist, it ignores the warning information in the log                                     |
...
| `dateext`                       | Uses date as the file suffix for log rotation                                                                   |
...
* `-v` - Displays the process of log rotation
...
`systemd` is an application used for initialization and takes over many system components. Use `journald` in `systemd` to take over the contents of the log.

`journald` is the daemon in `systemd` that takes over the logs. You need to use the `journalctl` command to read the logs.

Note that `journald` does not enable log persistence by default, which means it only retains and records all logs since startup. After the operating system restarts, deletion of historical logs occurs. All temporarily saved log files are in the **/run/log/journal/** directory by default.
...
This temporary file is binary. You must use the `journalctl` command to parse it.
...
* **facility** - This table shows the facility:
...
* `--since=DATA` or `-S` - Shows entries that are not older than the specified date. The date format is "YYYY-MM-DD HH:MM:SS". For example `journalctl --since="2025-04-24 14:00:30`
* `--until=DATA` or `-U` - Shows entries that are not newer than the specified date. The date format is "YYYY-MM-DD HH:MM:SS". For example `journalctl --since="2025-04-01 05:00:10" --until="2025-04-05 18:00:30"`
