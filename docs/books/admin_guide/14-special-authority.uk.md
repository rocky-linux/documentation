...
<font color=red>У всіх прикладах у цьому документі використовуються дії root, а дії звичайних користувачів коментуються окремо. У блоці коду markdown опис команди буде позначено символом # у попередньому рядку.</font>
...
| 5       | Кількість підкаталогів (включно з `.` та `..`). Для файлу він представляє кількість жорстких посилань, а 1 представляє себе.                                      |
...
|    **d**    | Представляє файл каталогу. За замовчуванням, по одному в кожному каталозі `.` та `..`.                                                                                                                                                            |
...
| :-------------------: | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|           4           | r (читання)  | Вказує на те, що ви можете прочитати цей файл. Ви можете використовувати такі команди, як `cat`, `head`, `more`, `less`, `tail` тощо. |
...
У Windows є аналогічна функція. Наприклад, щоб призначити користувачеві дозволи для файлу, для користувацького каталогу/файлу, **клацніть правою кнопкою миші** ---> **Properties** ---> **Security** ---> **Edit** ---> **Add** ---> **Advanced** ---> **Find now**, знайдіть відповідного користувача/групу ---> призначити певні дозволи ---> **apply** та завершіть.
...
Що означає "mask:: rwx" у вихідному повідомленні під час використання команди `getfacl`?
...
Тепер виникає запитання: якщо я створю новий файл у цьому каталозі, чи матиме він дозвіл ACL?
Відповідь – ні, оскільки щойно створений файл знаходиться після виконання команди `setfacl-m u:tom:rx -R /project`.
...
Чому GNU/Linux потрібні такі дивні дозволи?
...
Загальновідомо, що звичайні користувачі (uid >= 1000) можуть змінювати свої власні паролі. Справжній пароль зберігається у файлі **/etc/shadow**, але дозвіл файлу shadows дорівнює 000, а звичайні користувачі не мають жодних дозволів.
...
Оскільки звичайні користувачі можуть змінювати свій пароль, вони повинні були записати пароль у файл **/etc/shadow**. Коли звичайний користувач виконує команду `passwd`, він тимчасово змінює свій обліковий запис на власника файлу -- **root**. Для файлу **shadow** права доступу **root** не можуть бути обмежені. Ось чому команда `passwd` потребує дозволу SUID.
...
!!! warning
    ...
    `bash     # Suppose this is an executable binary file     Shell > vim suid.sh     #!/bin/bash     cd /etc && ls          Shell > chmod 4644 suid.sh     `
...
!!! warning
    ...
    `bash     Shell > find / -perm -4000 -a -type f -exec ls -l  {} \;     `
...
!!! warning
    ...
    `bash     # Suppose this is an executable binary file     Shell > touch sgid          Shell > chmod 2741 sgid     Shell > ls -l sgid     -rwxr-S--x  1 root root         0 Jan  14 12:11 sgid     `
...
!!! warning
    ...
    `bash     Shell > find / -perm -2000 -a -type f -exec ls -l  {} \;     `

### Sticky BIT

Використання команди <code>chattr</code> -- <code>chattr [ -RVf ] [ -v version ] [ -p project ] [ mode ] files...</code>
...
Чи може файл або каталог мати дозвіл **7755**?
...
| Частина | Опис                                                                                                                                                                                                                        |
| :-----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|    1    | Ім’я користувача або ім’я групи власників. Посилається на те, якому користувачу/групі надано дозволи. Якщо це група власників, потрібно написати "%", наприклад, **%root**. |
