...
- параметр `dateext` - Використовує дату як суфікс файлу для ротації журналу. Наприклад, під час першої ротації журналу старий файл журналу "secure" зміниться на "secure-20250424", а `logrotate` створить новий файл "secure"
...
- `-u` – Визначає один «одиницю» для перегляду відповідної інформації. Ви можете використовувати цей параметр кілька разів в одній команді. Наприклад, `journalctl -u crond.service -u sshd.service`
