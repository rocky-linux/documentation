...
:heavy_check_mark: **Перемістити** в межах системного дерева.  
:heavy_check_mark: **Створити** текстовий файл, **відобразити** його вміст та **змінити** його.  
:heavy_check_mark: **Використати** найкорисніші команди Linux.
...
Серед існуючих інтерпретаторів команд найчастіше використовується **Bourne-Again Shell** (`/bin/bash`). Вона призначається за замочуванням новим користувачам. З різних причин досвідчені користувачі Linux можуть вибрати альтернативні оболонки, такі як Korn Shell (`ksh`), C Shell (`csh`) тощо.

Каталог входу користувача за домовленістю зберігається в каталозі `/home` робочої станції. Він міститиме персональні дані користувача та конфігураційні файли його програм. За замовчуванням під час входу поточним каталогом обирається каталог входу.

Інсталяція типу робочої станції (з графічним інтерфейсом) запускає цей інтерфейс на терміналі 1. Оскільки Linux є багатокористувацькою системою, можливо одночасно підключити кількох користувачів до різних **фізичних терміналів** (TTY) або **віртуальних терміналів** (PTS). Віртуальні термінали доступні в графічному середовищі. Користувач перемикається з одного фізичного терміналу на інший за допомогою ++alt+"Fx"++ із командного рядка або за допомогою ++ctrl+alt+"Fx"++.
...
**Короткі опції** починаються з дефісу (`-l`), тоді як **довгі опції** починаються з двох дефісів (`--list`).
Подвійне дефіс (`--`) вказує на кінець списку опцій.
...
У літературі термін «опція» еквівалентний терміну «параметр», який частіше використовується в програмуванні. Необов'язкова частина опції або аргументу символізується шляхом укладання її в квадратні дужки `[` та `]`. Коли можливий більш ніж один варіант, їх розділяє вертикальна смуга, яка називається «канавкою» («piece»).
...
Після того, як інструкцію знайдено за допомогою `apropos` або `whatis`, її читає `man` («Man — твій друг»).
Цей набір посібників поділено на 8 розділів, згрупованих за темами, стандартним розділом є 1:
...
Переміщуйтеся в посібнику за допомогою стрілок ++↑++ та ++↓++. Для виходу з посібника натисніть клавішу ++q++.
...
Щоб негайно зупинити процес, використовуйте слово `now` замість часу. У цьому випадку, додаткове повідомлення не надсилається іншим користувачам системи.
...
| Клавіші             | Функція                                                                                   |
| ------------------- | ----------------------------------------------------------------------------------------- |
| ++exclam+exclam++   | Відкликає останню введену команду.                                        |
| ++exclam+n++        | Відкликає команду за номером у списку.                                    |
| ++exclam+"string"++ | Викликає останню команду, яка починається з рядка.                        |
| ++arrow-up++        | Переміщується у вашій історії, повертаючись у часі від останньої команди. |
| ++arrow-down++      | Переміщується у вашій історії, рухаючись вперед у часі.                   |
...
- Натисніть клавішу ++tab++, щоб завершити запис у випадку одного рішення.
- У випадку з множинними рішеннями натисніть ++tab++ другий раз, щоб побачити варіанти.

Якщо подвійне натискання клавіші ++tab++ не відображає варіанти, тоді немає розв'язку з поточним завершенням.

## Відображення та ідентифікація

### Команда `clear`

Команда `clear` очищає екран терміналу. Точніше, він зміщує дисплей так, щоб командний рядок знаходився у верхній частині екрана в першому рядку.
...
!!! Tip "Порада"
    ...
    ```
    ++control+l++ матиме такий же ефект, що й команда `clear`
    ```
...
З різних причин розробнику скриптів може знадобитися використовувати спеціальні послідовності (що починаються з символу `\`). У цьому випадку буде вказано опцію `-e`, що дозволить інтерпретацію послідовностей.
...
Команда `date` відображає дату та час. Команда має наступний синтаксис:
...
В цьому останньому прикладі параметр `-d` відображає задану дату. Опція `+%j` форматує цю дату так, щоб відображався лише день року.
...
Команда `date` також дозволяє змінити системну дату та час. У цьому випадку буде використано опцію `-s`.
...
Команда `id` відображає інформацію про користувачів та групи. За замовчуванням параметр користувача не додається, і відображається інформація про поточного користувача та групу
...
**Каталог підключення** – це робочий каталог, пов’язаний з користувачем. Каталоги входу за замовчуванням зберігаються в каталозі `/home`.
...
Таким чином, відносний шлях може починатися з `./` або `../`. Коли відносний шлях посилається на підкаталог або файл у поточному каталозі, то `./` часто пропускається. Згадування першого `./` у дереві буде дійсно потрібним лише для запуску виконуваного файлу.
...
- За **абсолютним шляхом** поточний каталог не має значення. Ми починаємо з кореневого каталогу та просуваємося до каталогів `home`, `groupS`, `alice` і, нарешті, до файлу `myfile`: `/home/groupS/alice/myfile`.
- За **відносним шляхом**, починаючи з поточного каталогу `bob`, ми піднімаємося на один рівень вгору через `..` (тобто до каталогу `groupA`), потім вниз до каталогу alice і, нарешті, до файлу `myfile`: `../alice/myfile`.
...
| Опція | Опис                                                                                                                                  |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `-a`  | Відображає всі файли, навіть приховані. Приховані файли в Linux - це ті, що починаються з `.`.        |
...
| `-h`  | Відображає розміри файлу у найбільш доцільному форматі (байт, кілобайт, мегабайт, ...). `h` означає «читабельний для людини». Потребує використання з -l опцією.                                                                                                                                                                                                                                                                                             |
| `-s`  | Відображає розмір виділеного файлу в блоках. У команді `ls` розмір одного блоку за замовчуванням становить 1024 байти. В операційній системі GNU/Linux «блок» є найменшою одиницею зберігання у файловій системі, і загалом один блок дорівнює 4096 байтам. В операційній системі Windows, беручи за приклад файлову систему NTFS, її найменша одиниця зберігання називається «Кластер». Визначення мінімального імені одиниці зберігання може відрізнятися залежно від різних файлових систем. |
| `-А`  | Відображає усі файли у каталозі за винятком `.` і `..`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `-R`  | Відображає вміст підкаталогів рекурсивно.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `-F`  | Відображає тип файлів. Виводить `/` для каталогу, `*` для виконуваних файлів, `@` для символічного посилання та нічого для текстового файлу.                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `4`             | Кількість підкаталогів (включно з `.` та `..`). Для файлу це кількість жорстких посилань, і 1 представляє сам себе.                                                              |
| `rockstar`      | Власність користувача.                                                                                                                                                                                              |
| `rockstar`      | Власність групи.                                                                                                                                                                                                    |
| `4096`          | Для файлів, показує розмір файлу. Для каталогів відобразиться фіксоване значення 4096 байт, окуповане іменуванням файлу. Щоб обчислити загальний розмір каталогу, використовуйте `du -sh rockstar/` |
...
Команда `ls` має багато опцій. Ось кілька продвинутих прикладів використання:
...
- Список файлів `/var` розміром більше 1 мегабайта, але менше 1 гігабайта. У цьому прикладі використовуються розширені команди `grep` з регулярними виразами. Новачкам не доведеться надто мучитися, у майбутньому буде спеціальний підручник, щоб представити ці регулярні вирази.
...
Наведена вище команда відобразить вміст папки (всередині) за замовчуванням. Для самої папки можна використовувати опцію `-d`.
...
За замовчуванням команда `ls` не відображає останню скісну риску папки.
У деяких випадках, як, наприклад, для сценаріїв, корисно їх відобразити:
...
В іншому випадку слід використовувати опцію `-p`. Опція `-p` створює батьківські каталоги, якщо вони не існують.
...
| `-r`      | Видалити каталог і рекурсивно видалити його підкаталоги. |
...
Ім'я файлу з двома типами дисків починається з символу `-`. Без використання `--` оболонка інтерпретувала б `-d` у `-hard-hard` як опцію.
...
Перейменовує `file1` на `file2`. Якщо `file2` вже існує, замініть вміст файлу на `file1`.
...
За допомогою клавіші ++enter++ переміщайте рядок за рядком. За допомогою клавіші ++space++ переміщайте сторінку за сторінкою. `/text` дозволяє шукати входження у файл.
...
| ---------------------------------------------------------- | ----------------------------------------------------------------- |
| ++"h"++ або ++h++                                          | Допомогти.                                        |
| ++arrow-up++ ++arrow-down++ ++arrow-right++ ++arrow-left++ | Рухатися вгору, вниз лінією праворуч або ліворуч. |
| ++enter++                                                  | Посунути вниз на один рядок.                      |
| ++space++                                                  | Посунути на одну сторінку.                        |
| ++page-up++ та ++page-down++                               | Посунути вгору або вниз одну сторінку.            |
| ++"g"++ та ++g++                                           | Посунути на перші та останні сторінки                             |
| `/text`                                                    | Пошук тексту.                                     |
| ++"q"++ або ++q++                                          | Закриває команду `less`.                          |
...
Команда `tac` виконує майже протилежні дії до `cat`. Вона відображає вміст файлу, починаючи з кінця (що особливо цікаво для читання журналів!).
...
| Опція  | Опис                          |
| ------ | ----------------------------- |
| `-n х` | Показує перші `x` рядки файлу |
...
З опцією `-f` інформація про зміни у файлі завжди виводиться, якщо користувач не виходить зі стану моніторингу за допомогою ++control+c++. Ця опція дуже часто використовується для відстеження файлів журналів (логів) в реальному часі.
...
| `-r`      | Зворотний порядок результату. Використовується разом з опцією `-n` для сортування від найбільшого до найменшого.                           |                                 |
| `-u`      | Прибирає дублікати після сортування. Еквівалентно `sort FILE`                                                                                              | команда uniq\`. |

Команда `sort` сортує файл лише на екрані. Файл не змінюється у сортуванні. Щоб зберегти сортування, використовуйте опцію `-o` або перенаправлення виводу `>`.

За замовчуванням числа відсортовані відповідно до їх значення. Таким чином, «110» буде перед «20», яке саме перед «3». Необхідно вказати опцію `-n`, щоб блоки числових символів сортувалися за їхнім значенням.
...
Символ `*` замінює рядок, що складається з будь-яких символів. Символ `*` також може позначати відсутність символу.
...
**stderr** отримує потоки помилок, повернуті командою. Інші потоки спрямовуються на **stdout**.
...
Перенаправити вхідний потік з іншого файлу можна за допомогою символу `<` або `<<`. Команда прочитає файл замість клавіатури:
...
Коли обидва вихідні потоки перенаправляються, інформація на екрані не відображається. Щоб використовувати як перенаправлення виводу, так і зберегти відображення, вам потрібно буде скористатися командою `tee`.

### Роздільники

**Роздільник** — це механізм, який дозволяє зв’язати стандартний вихід першої команди зі стандартним входом другої команди.

Цей зв'язок є односпрямованим і використовує символ `|`. Символ вертикальної риски `|` отримується одночасним натисканням клавіш ++shift+bar++.
...
Команда `alias` виводить список псевдонімів для поточного сеансу. У дистрибутивах Linux псевдоніми встановлюються за замовчуванням. Тут псевдоніми для сервера Rocky:
...
Усі команди запускатимуться послідовно в порядку введення, коли користувач натисне ++enter++.
...
:heavy_check_mark: Яка команда перенаправляє потік помилки на файл `errors.log`?
