...
:heavy_check_mark: деякі корисні команди, не висвітлені в попередньому розділі.  
...
Команда `uniq` — це дуже потужна команда, яка використовується разом із командою `sort`, особливо для аналізу файлів журналів. Це дозволяє сортувати та відображати записи, видаляючи дублікати.
...
Команда `xargs` очікує на вхідні дані зі стандартного вводу **stdin**. Введено три рядки. Кінець введеного користувачем поля визначається для `xargs` за допомогою послідовності натискань клавіш ++ctrl+d++. Потім `xargs` виконує команду за замовчуванням `echo`, а потім передає три аргументи, що відповідають введеним користувачем даним, а саме:
...
У цьому випадку команда `xargs` має бути примусово виконана для виконання команди `find` кілька разів (один раз на рядок, що вводиться як стандартний ввід). Опція `-L`, а потім **ціле число**, дозволяє вказати максимальну кількість записів, які будуть оброблені командою одночасно:
...
Особливістю команди `xargs` є те, що вона розміщує вхідний аргумент в кінці викликаної команди. Це дуже добре працює з наведеним вище прикладом, оскільки передані файли формуватимуть список файлів, які потрібно додати до архіву.

Використовуючи приклад команди `cp`, для копіювання списку файлів у каталозі, цей список файлів буде додано в кінці команди... але те, що команда `cp` очікує в кінці команди, це пункт призначення. Для цього скористайтеся опцією `-I`, щоб розмістити вхідні аргументи деінде, ніж в кінці рядка.
...
Команда `yumdownloader` завантажує RPM-пакети з репозиторіїв.  Еквівалентно `dnf download --downloadonly --downloaddir ./ назва-пакета`
...
| `-p`                         | зберігає позначки часу вихідних файлів                                           |
...
**atime** – Після доступу до вмісту файлу за допомогою таких команд, як `cat`, `less`, `more` та `head`, значення `atime` файлу можна оновити. Будь ласка, зверніть увагу! Значення `atime` файлу не оновлюється в режимі реального часу, і з міркувань продуктивності йому потрібно зачекати певний час, перш ніж його можна буде відобразити.
**mtime** – Зміна вмісту файлу може оновлювати його `mtime` (наприклад, додавання або перезапис вмісту файлу через перенаправлення), оскільки розмір файлу є його властивістю, `ctime` також буде оновлено одночасно.
**ctime** – Зміна власника, групи, дозволів, розміру файлу та посилань (м’яких та жорстких) файлу призведе до оновлення ctime.

Для каталогів:

**atime** – Після використання команди `cd` для входу до нового каталогу, до якого раніше ніколи не зверталися, ви можете оновити та виправити `atime` цього каталогу.
**mtime** – Виконання таких операцій, як створення, видалення та перейменування файлів у цьому каталозі, призведе до оновлення `mtime` та `ctime` каталогу.
**ctime** – Коли зміниться дозвіл, власник, група тощо каталогу, `ctime` каталогу буде оновлено.
