...
| 6  | 所有者用户名称。                                                         |
| 7  | 所属组名称。                                                           |
| 8  | 对于文件，它显示文件的大小。 对于目录，它显示了文件名占用的固定 4096 字节。 要计算目录的总大小，请使用 `du -sh` |
| 9  | 最后修改日期。                                                          |
| 10 | 文件（或目录）的名称。                                                      |
...
ACL（Access Control List，访问控制列表）解决了 Linux 下三种身份无法满足资源权限分配需求的问题。

例如，老师给学生们上课，并在操作系统的根目录下创建一个目录。 只有本班的学生才被允许上传和下载；其他学生则不能这样做。 此时，目录的权限为770。 有一天，另一所学校的一名学生来听课，问老师权限应该如何分配。 如果您将此学生放入到 **所属组** 中，他将拥有与此班学生相同的权限 - **rwx**。 若将学生放入到 **其他用户** 组中，他将没有任何权限。 此时，默认的权限分配无法满足要求；您需要使用 ACL。
...
您需要找到设备的文件名和挂载点的分区号。 例如，在我的机器上，你可以这样做：
...
**mask** 指定了最大有效权限。 授予用户的权限并非真正的权限；真正的权限只能通过将用户的权限与掩码权限进行 "逻辑与" 运算来获得。
...
!!! info "信息"
    ...
    因为默认 mask 是 rwx，所以任何用户的 ACL 权限的结果都是他们自己的权限。
...
对于 ACL 权限，这意味着当父目录设置 ACL 权限时，所有子目录和子文件都会继承这些权限。
...
!!! info "信息"
    ...
    使用 ACL 权限的默认和递归选项时，`setfacl` 命令必须作用于目录。 如果操作对象是文件，将输出错误提示。
...
由于普通用户可以更改密码，所以系统必须将密码写入到 **/etc/shadow** 文件中。 当普通用户执行 `passwd` 命令时，它将临时更改为文件的所有者 -- **root**。 对于 **shadow** 文件而言，**root** 不受权限限制。 这就是为什么 `passwd` 命令需要 SUID 权限。
...
SGID 由数字 **2** 进行表示，因此 `locate` 命令的权限为 2711。
...
SGID 不仅可用于可执行二进制文件/程序，还可用于目录，不过这种用法较为少见。
...
* 仅对目录有效。
* 普通用户对此目录具有 w 和 x 权限。
* 如果没有 Sticky Bit ，拥有 w 权限的普通用户可以删除此目录中的所有文件（包括其他用户创建的文件）。 一旦目录被授予了 SBIT 权限，只有 root 用户可以删除所有文件。 即使普通用户有 w 权限，也只能删除自己的文件（其他用户创建的文件不能删除）。
...
|:--:|:-----------------------:|:--------------------:|:--------------------:|:--------------------:|:----:|
| 文件 |            ×            |          ×           |          ×           |          √           |  -   |
| 目录 | x <br>（目录和目录下的文件） | √ <br>（目录中的文件） | √ <br>（目录中的文件） | √ <br>（目录中的文件） |  ×   |
...
|:--:|:-----------------------:|:--------------------:|:--------------------:|:--------------------:|:----:|
| 文件 |            ×            |          ×           |          √           |          √           |  -   |
| 目录 | x <br>（目录和目录下的文件） | x <br>（目录中的文件） | √ <br>（目录中的文件） | √ <br>（目录中的文件） |  √   |
...
我们知道，在 GNU/Linux 操作系统中，只有管理员 root 用户才有权限使用位于 **/sbin/** 和 **/usr/sbin/** 目录中的命令。 一般来说，一家公司会有一个团队来维护一组服务器。 这组服务器可以指代单个地理位置的单个机房，也可以指代多个地理位置的机房。 团队负责人使用 root 用户的权限，而其他团队成员可能只有普通用户的权限。 由于负责人工作繁忙，无暇维护服务器的日常运行，因此大部分工作都落在了普通用户身上。 然而，普通用户在使用命令时有很多限制，此时就需要使用 sudo 权限。

若要向普通用户授予权限，**您必须使用 root 用户（uid=0）**。

您可以通过使用 `visudo` 命令来授权普通用户。 您实际更改的是 **/etc/sudoers** 文件。
...
您可以使用 Rust 版本的 sudo 作为替代方案。 有关更多详细信息，参阅 [此处](https://github.com/trifectatechfoundation/sudo-rs)。
