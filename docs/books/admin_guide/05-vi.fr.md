...
!!! abstract Tip A line of text is ended by pressing <kbd>ENTER</kbd> but if the screen is not wide enough, VI makes automatic line breaks, _wrap_ configuration by default. These line breaks may not be desired, this is the _nowrap_ configuration.
...
!!! abstract Note VI switches to *insertion* mode. So you will have to press the <kbd>ESC</kbd> key to return to *command* mode.
...
!!! abstract Note The <kbd>R</kbd> command switches to *replace* mode, which is a kind of *insert* mode.
