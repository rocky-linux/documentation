...
- **Знищення**: добровільне чи примусове. Людські або технічні. Вірус, ...
- **Видалення**: добровільне чи примусове. Людські або технічні. Вірус, ...
...
- **Холодне резервне копіювання**: резервне копіювання, коли система вимкнена або перебуває в стані технічного обслуговування.  Резервні копії даних точно збігаються з даними в системі протягом цього періоду.
- **Гаряче резервне копіювання**: резервне копіювання відбувається, коли система працює нормально.  Оскільки дані в системі оновлюються в будь-який час, резервні копії даних мають певну затримку відносно реальних даних системи.
...
Команди, які ми тут використовуватимемо, це `tar` та `cpio`. Якщо ви хочете дізнатися більше про інструмент `dump`, зверніться до [цього документа](../../guides/backup/dump_restore.md).
...
**Реплікація**: технологія резервного копіювання, яка копіює набір даних з одного джерела даних до іншого або кількох джерел даних, яка в основному поділяється на **синхронну реплікацію** та **асинхронну реплікацію**. Це розширена частина резервного копіювання для системних адміністраторів-початківців, тому в цьому базовому документі не буде докладно описувати цей вміст.
...
- властивості або характеристики файлів або каталогів у файлі резервної копії будуть частково збережені. Наприклад, значення mtime для файлів або каталогів буде збережено, але номер `inode` не буде збережено.
...
`tar` неявно створює резервні копії у відносному режимі, навіть якщо шлях до інформації, яку потрібно резервувати, згаданий в абсолютному режимі. Проте резервне копіювання та відновлення в абсолютному режимі можливі. Якщо ви хочете побачити окремий приклад використання `tar`, будь ласка, зверніться до [цього документа](../../guides/backup/tar.md).
...
Утилітою за замовчуванням для створення резервних копій у системах UNIX є команда `tar`. Ці резервні копії можна стиснути за допомогою `bzip2`, `xz`, `lzip`, `lzma`, `lzop`, `gzip`, `compress` або `zstd`.
...
Ви не можете перевірити стиснутий архів за допомогою клавіші `W`. Замість цього потрібно використовувати клавішу `d`.
...
Попередня команда витягує лише файл `/шлях/до/файлу` з резервної копії `backup.tar`. Цей файл буде відновлено до каталогу `/шлях/до/`, створеного або вже присутнього в активному каталозі.
...
##### Витягнути групу файлів із резервної копії _tar_ за допомогою регулярних виразів
...
- \*_--wildcards _.conf__ відповідає файлам із розширенням `.conf`.
...
На відміну від команди `tar`, немає можливості одночасно створювати резервні копії та стиснення.
Отже, це робиться в два етапи: резервне копіювання та стиснення.

`cpio` має три режими роботи, кожен з яких відповідає окремій функції:

1. **режим копіювання** – створює резервну копію (архів). Увімкніть цей режим за допомогою опцій `-o` або `--create`. У цьому режимі ви повинні згенерувати список файлів за допомогою певної команди (`find`, `ls` або `cat`) та передати його до cpio.
...
2. **режим копіювання** – витягує файли з архіву. Ви можете ввімкнути цей режим за допомогою опції `-i`.

3. **режим копіювання з передачею** – копіює файли з одного каталогу в інший. Ви можете ввімкнути цей режим за допомогою опцій `-p` або `--pass-through`.
...
Результат виконання команди `find` надсилається як вхідні дані для команди `cpio` через _pipe_ (символ `|`, + +зсув вліво+зворотна коса риска++).
...
На відміну від команди `tar`, немає можливості одночасно зберігати та стискати.
Отже, це робиться в два етапи: збереження та стиснення.

Синтаксис першого методу легше зрозуміти і запам'ятати, оскільки він складається з двох кроків.

Для першого методу файл резервної копії автоматично перейменовується утилітою `gzip`, яка додає `.gz` до кінця імені файлу. Аналогічно, утиліта `bzip2` автоматично додає `.bz2`.
...
Довгий параметр `no-absolute-filenames` дозволяє відновлення у відносному режимі. Дійсно, символ `/` на початку шляху буде видалено.
