...
У 2016 році `systemd` виявила вразливість безпеки, яка дозволяла будь-якому непривілейованому користувачеві здійснити «атаку відмови в обслуговуванні» проти `systemd`.
...
Команда `systemctl` є основним інструментом для керування `systemd` та поєднує попередні команди `service` та `chkconfig`.
