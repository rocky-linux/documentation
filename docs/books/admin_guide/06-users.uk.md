...
| `-d каталог`        | Домашній каталог.                                                                                                                                                                      |
| `-s оболонка`       | Оболонка.                                                                                                                                                                              |
| `-c КОМЕНТАР`       | Додати коментар.                                                                                                                                                                       |
| `-U`                | Додає користувача до групи з однаковою назвою, створеної одночасно. Якщо цей параметр не вказано за замовчуванням, під час створення користувача буде створено групу з такою ж назвою. |
| `-M`                | Не створюйте домашній каталог користувача.                                                                                                                                             |
| `-r`                | Створіть системний обліковий запис.                                                                                                                                                    |
...
| `-f`          | Кількість днів після закінчення терміну дії пароля до вимкнення облікового запису. |
| `-e`          | Дата вимкнення облікового запису.                                                  |
...
| Опція           | Опис                                                                                                                                                                                                                           |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `-m`            | Пов’язаний із параметром `-d`, переміщує вміст старого каталогу входу до нового. Якщо старого домашнього каталогу не існує, новий домашній каталог не буде створено; Якщо новий домашній каталог не існує, він буде створений. |
| `-l login`      | Нове ім'я для входу. Після того, як ви змінили ім’я для входу, вам також потрібно змінити ім’я домашнього каталогу, щоб воно відповідало йому.                                                                                 |
| `-e YYYY-MM-DD` | Термін дії облікового запису.                                                                                                                                                                                                  |
| `-L`            | Назавжди заблокувати обліковий запис. Тобто `!` додається на початку поля пароля `/etc/shadow`                                                                                                                                 |
...
| Опція | Опис                                                                                              |
| ----- | ------------------------------------------------------------------------------------------------- |
| `-r`  | Видалити домашній каталог користувача та поштові файли, розташовані в каталозі `/var/spool/mail/` |
...
| `-l`      | Обліковий запис користувача завжди заблоковано. Лише для root (uid=0).                          |
...
| `-n DAYS` | Мінімальний термін дії пароля. Постійна зміна. Лише для root (uid=0).                           |
| `-x DAYS` | Максимальний термін дії пароля. Постійна зміна. Лише для root (uid=0).                          |
| `-w DAYS` | Час попередження до закінчення терміну дії. Постійна зміна. Лише для root (uid=0).              |
| `-i DAYS` | Затримка перед деактивацією, коли пароль буде вичерпано. Постійна зміна. Лише для root (uid=0). |
...
| Опція            | Опис                                                                                                                   |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `-I DAYS`        | Затримка перед деактивацією, термін дії пароля закінчився. Постійна зміна.                                             |
| `-l`             | Відображає деталі політики.                                                                                            |
| `-m DAYS`        | Мінімальний термін дії пароля. Постійна зміна.                                                                         |
| `-M DAYS`        | Максимальний термін дії пароля. Постійна зміна.                                                                        |
| `-d LAST_DAY`    | Останній пароль змінено. Ви можете використовувати стиль часової позначки дня або стиль YY-MM-DD. Постійна зміна.      |
...
!!! Порада
    ...
    You can use the `exit`/`logout` command to exit users who have been switched. Слід зазначити, що після зміни користувача не буде нової `дочірньої оболонки` або `підоболонки`, наприклад:
