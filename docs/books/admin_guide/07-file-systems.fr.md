...
!!! abstract Warning There can be only one extended partition per physical disk. In order to benefit from additional drives, the extended partition can be split into logical partitions
...
!!! abstract Warning Beware, the extended partition, which does not support a file system, still has a number.
...
!!! abstract Note LVM is only managed by the operating system. Therefore the _BIOS_ needs at least one partition without LVM to boot.
...
!!! abstract Tip Striping_ improves performance by writing data to a predetermined number of physical volumes with a _round-robin_ technique.
...
!!! abstract Warning Without a file system it is not possible to use the disk space.
...
!!! abstract Note The only exception is the **swap** partition.
...
!!! abstract Note An inode number is unique within a file system.
...
!!! abstract Note The size of the inode table determines the maximum number of files the FS can contain.
...
!!! abstract Danger In the event of a sudden stop, the file system may lose its consistency and cause data loss.
...
!!! abstract Warning The partition to be checked must be unmounted.
...
!!! abstract Note In Linux everything is a file.
...
!!! abstract Note Lines are read sequentially (`fsck`, `mount`, `umount`).
...
!!! abstract Warning Only the mount points listed in `/etc/fstab` will be mounted on reboot.
...
!!! abstract Note The `mount` command alone displays all mounted file systems.
...
!!! abstract Note When disassembling, you must not stay below the mounting point. Otherwise, the following error message is displayed: `device is busy`.
...
!!! abstract Note While there is nothing technically wrong with creating a file or directory with a space in it, it is generally a "best practice" to avoid this and replace any space with an underscore.

!!! abstract Note The **.** at the beginning of the file name only serves to hide it from a simple `ls`.

!!! abstract Warning Under Linux, the extension of a file is not a necessary reference to open or modify it. However, it can be useful for the user.
...
!!! abstract Warning If the source file is destroyed, the counter is decremented and the link file still accesses the file.
...
!!! abstract Warning If the source file is destroyed, the link file can no longer access the file.
...
!!! abstract Warning The rights associated with files differ from those associated with directories (see below).
...
!!! abstract Note Moving or renaming a file depends on the rights of the target directory. Deleting a file depends on the rights of the parent directory.
...
!!! abstract Note Permissions apply to **u**ser, **g**roup and **o**ther (**ugo**) depending on the owner and group.
...
!!! abstract Warning The rights of files and directories are not dissociated. For some operations, it will be necessary to know the rights of the directory containing the file. A write-protected file can be deleted by another user as long as the rights of the directory containing it allow this user to perform this operation.
...
!!! abstract Danger `S`, `S` and `T` in capital letters **if the right does not exist**.
...
!!! abstract Note The identity of the user requesting the execution of the order is no longer taken into account.
...
!!! abstract Warning It is not possible to pass the _SUID_ or _SGID_ to a shell script. The system does not allow it because it is too dangerous for security!
...
!!! abstract Warning `umask` does not affect existing files.

!!! abstract Note `umask` modifies the mask until the disconnection.
...
!!! abstract Warning `umask -S` displays the file rights (without the execute right) of the files that will be created. So it is not the display of the mask used to subtract the maximum value.

!!! abstract Tip The `umask` command being a _bash_ command, (a `type umask` returns `umask is a shell primitive`) you have to search `umask` in `man bash`.
