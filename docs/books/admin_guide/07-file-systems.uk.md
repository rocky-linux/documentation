...
- _Основний розділ_ (або головний розділ)
- _Розширений розділ_
...
Існує щонайменше дві команди для розбиття диска на розділи: `fdisk` та `cfdisk`. Обидві команди мають інтерактивне меню. `cfdisk` є надійнішим та краще оптимізованим, тому найкраще використовувати саме його.

Єдина причина використовувати `fdisk` — це коли ви хочете переглянути список усіх логічних пристроїв за допомогою опції `-l`. Команда `fdisk` використовує таблиці розділів MBR, тому вона не підтримується для таблиць розділів **GPT** і не може бути оброблена для дисків розміром більше **2 ТБ**.
...
Команда `parted` (_редактор розділів_) може розділити диск без недоліків `fdisk`.

Команду `parted` можна використовувати в командному рядку або інтерактивно. Вона також має функцію відновлення, здатну переписати видалену таблицю розділів.

```bash
parted [-l] [device]
```

У графічному інтерфейсі є дуже потужний інструмент `gparted`: _G_nome _PAR_tition _ED_itor.
...
Розділ, створений **стандартним розділом**, не може динамічно налаштовувати ресурси жорсткого диска, після монтування розділу ємність повністю фіксована, це обмеження неприйнятне на сервері. Хоча стандартний розділ можна примусово розширити або зменшити за допомогою певних технічних засобів, це легко спричиняє втрату даних. LVM може дуже добре вирішити цю проблему. LVM доступний у Linux з ядром версії 2.4, і його основні функції:
...
**Фізичний носій**: Носієм інформації LVM може бути весь жорсткий диск, розділ диска або RAID-масив. Перш ніж можна буде виконувати подальші операції, пристрій необхідно перетворити або ініціалізувати у фізичний том LVM (**PV**).

**PV (Фізичний Том)** – це базовий логічний блок сховища LVM. Для створення фізичного тому можна використовувати розділ диска або сам диск.

**VG (Volume Group)**: Подібно до фізичних дисків у стандартному розділі, VG складається з одного або кількох PV.

**LV (Логічний том)**: Подібно до розділів жорсткого диска у стандартних розділах, LV будується поверх VG. Ви можете налаштувати файлову систему на LV.

<b><font color="blue">PE</font></b>: Найменша одиниця пам’яті, яку можна виділити у фізичному томі, за замовчуванням <b>4 МБ</b>. Ви можете вказати додатковий розмір.

<b><font color="blue">LE</font></b>: Найменша одиниця пам’яті, яку можна виділити в логічному томі. У однакових VG PE і LE однакові і відповідають одна до одної.
...
Команда `vgcreate` створює групи томів. Вона групує один або кілька фізичних томів у групу томів.
...
| Опція        | Опис                                                                                                                                                                                                          |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-L size`    | Встановлює розмір логічного тому в K, M або G.                                                                                                                                                |
| `-n name`    | Назва LV. У `/dev/name_volume` було створено спеціальний файл з цією назвою.                                                                                                  |
| `-l  number` | Крім використання одиниці ємності жорсткого диска, можна також використовувати кількість PE. Також можна використовувати кількість PE. Один PE дорівнює 4 МБ. |
...
Кожна файлова система має структуру, яка є ідентичною для кожного розділу. Система ініціалізує **завантажувальний сектор** та **суперблок**, а потім адміністратор ініціалізує **таблицю індексних дескрипторів** та **блок даних**.
...
Розмір таблиці **Суперблок** визначається під час створення. Він присутній на кожній перегородці і містить елементи, необхідні для її використання.
...
Копія завантажується в центральну пам'ять, як тільки система ініціалізується. Ця копія оновлюється одразу після зміни, і система періодично зберігає її (команда `sync`).

Коли система зупиняється, вона також копіює цю таблицю в пам'яті в свій блок.

### Inodes таблиця

Розмір **таблиці inode** визначається під час її створення та зберігається на розділі. Він складається із записів, званих inode, що відповідають створеним файлам. Кожен запис містить адреси блоків даних, що утворюють файл.
...
Копія завантажується в центральну пам'ять, як тільки система ініціалізується. Ця копія оновлюється одразу після її зміни, і система періодично зберігає її (команда `sync`).
...
Інформація, яку містить _таблиця inode_:
...
**Щоб гарантувати узгодженість файлової системи**, образ суперблоку та таблиці inode завантажується в оперативну пам'ять (RAM) під час завантаження операційної системи, щоб усі операції вводу/виводу виконувалися через ці системні таблиці. Коли користувач створює або змінює файли, цей образ пам’яті оновлюється першим. Тому операційна система повинна регулярно оновлювати суперблок логічного диска (команда `sync`).
...
У разі виявлення помилок пропонуються рішення для усунення невідповідностей. Після відновлення файли, що залишаються без записів у таблиці inode, приєднуються до папки `/lost+found` логічного диска.
...
Файл `/etc/fstab` зчитується під час запуску системи та містить монтування, яке потрібно виконати. Кожна файлова система, яку потрібно змонтувати, описується в одному рядку, поля розділені пробілами або символами табуляції.
...
| 5       | Увімкніть або вимкніть керування резервним копіюванням (0:без резервного копіювання, 1:резервне копіювання). Команда `dump` тут використовується для резервного копіювання. Ця застаріла функція спочатку була розроблена для резервного копіювання старих файлових систем на стрічку. |
| 6       | Перевірте порядок під час перевірки FS командою `fsck` (0:без перевірки, 1:пріоритет, 2:не пріоритет)                                                                                                                                                                                                                  |

Команда `mount -a` дозволяє автоматично монтувати систему на основі вмісту конфігураційного файлу `/etc/fstab`. Потім змонтована інформація записується в `/etc/mtab`.
...
У кожному каталозі є два приховані файли: **.** та **..**. Наприклад, для перегляду потрібно використовувати `ls -al`:
...
Для зв'язку з периферійними пристроями (жорсткими дисками, принтерами тощо) Linux використовує файли інтерфейсу, які називаються спеціальними файлами (_файл пристрою_ або _спеціальний файл_). Вони дозволяють ідентифікувати периферійні пристрої.
...
- **Файли каналів** передають інформацію між процесами за принципом FIFO (_Перший прийшов, перший вийшов_).
  Один процес записує тимчасову інформацію у файл _pipe_, а інший її зчитує. Після прочитання інформація більше не доступна.

- **Сокет-файли** дозволяють двонаправлений міжпроцесний зв'язок (на локальних або віддалених системах). Вони використовують _inode_ файлової системи.
...
| Типи посилань            | Опис                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| файл м'якого посилання   | Цей файл схожий на ярлик для Windows. Він має дозвіл 0777 і вказує на вихідний файл. Коли оригінальний файл видалено, ви можете скористатися командою `ls -l` для перегляду вихідної інформації файлу програмного посилання. У вихідній інформації назва файлу програмного посилання відображається червоним кольором, а загострений оригінальний файл відображається червоним кольором із блимаючим запитом. |
| Файл жорсткого посилання | Цей файл представляє різні маппінги, що займають один і той самий номер _inode_. Їх можна оновлювати синхронно (включаючи вміст файлу, час модифікації, власника, приналежність до групи, час доступу тощо). Жорстко пов’язані файли не можуть охоплювати розділи та файлові системи та не можуть використовуватися в каталогах.                                                                           |
...
| :---------------------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|            r            | Читання. Дозволяє читати файл (`cat`, `less`, ...) та копіювання файлу (`cp`, ...). |
|            w            | Писати. Дозволяє змінювати вміст файлу (`cat`, `>>`, `vim`, ...).                                                                                      |
|            x            | Виконати. Розглядає файл як e**X**ecutable (двійковий файл або скрипт).                                                                                                                                |
...
|         w        | Писати. Дозволяє створювати та видаляти файли/каталоги в цьому каталозі, такі як команди `mkdir`, `rmdir`, `rm`, `touch` тощо. |
...
!!! info
    ...
    У деяких командах ви можете використовувати _a_ (_all_) для представлення _ugo_. Наприклад: `chmod a+x FileName` еквівалентно `chmod u+x,g+x,o+x FileName` або `chmod ugo+x FileName`.

### Управління атрибутами

Відображення прав здійснюється за допомогою команди `ls -l`. Це останні 9 символів блоку з 10. Точніше 3 рази по 3 символи.
...
За замовчуванням, _власником_ файлу є той, хто його створив. _Група_ файлу — це група власника, який створив файл. _Інші_ — це ті, кого не стосуються попередні випадки.
...
!!! info
    ...
    `bash     # root user     Shell > touch a.txt     Shell > ll     -rw-r--r-- 1 root root     0 Oct  8 13:00 a.txt     `
...
Коли вищезазначений файл записується, він фактично перевизначає параметр **UMASK** файлу `/etc/login.defs`. Якщо ви хочете покращити безпеку операційної системи, ви можете встановити для umask значення **027** або **077**.
