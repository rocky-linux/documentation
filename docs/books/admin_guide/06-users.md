...
| Option | Description                                    |
| ------ | ---------------------------------------------- |
| `-I DAYS` | Defines days to delay before deactivation, password expired. Permanent change.|
...
| `INACTIVE` | Defines number of days of grace after password expiration. Corresponds to the 7th field of the `/etc/shadow` file. `-1` value means that the grace period feature is turned off.|
| `EXPIRE`   | Defines account expiration date. Corresponds to the 8th field of the `/etc/shadow` file.                                                |
| `SHELL`    | Defines command interpreter.                                                     |
