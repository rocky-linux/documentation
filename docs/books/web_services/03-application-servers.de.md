...
In Rocky Linux 10 gibt es, wie auch in seinem Upstream, keine Module. Dies bedeutet, dass Sie bei der Installation von PHP das erhalten, was das Appstream-Repository enthält. Um herauszufinden, was das ist, verwenden Sie diesen Befehl:
...
Ihre Version kann abweichen, wenn Sie eine neuere Version von 10 verwenden.

### Installation des PHP CGI-Modus

Installieren und verwenden Sie zunächst PHP im CGI-Modus. Es kann nur zusammen mit dem Apache-Webserver und seinem Modul `mod_php` funktionieren. Der FastCGI-Teil (php-fpm) dieses Dokuments erklärt, wie PHP in Nginx (aber auch Apache) integriert wird.

Die Installation von PHP ist relativ einfach. Sie besteht aus der Installation des Hauptpakets und der wenigen Module, die Sie benötigen.

Das folgende Beispiel installiert PHP mit den Modulen, die normalerweise damit auch installiert sind.
...
Um PHP-Seiten im CGI-Modus bereitzustellen, müssen Sie den Apache-Server installieren, ihn konfigurieren, aktivieren und starten.
...
Der Standard-`vhost` sollte auf Anhieb funktionieren. PHP bietet eine Funktion `phpinfo()`, die eine Übersichtstabelle seiner Konfiguration generiert. Es ist hilfreich zu testen, ob PHP einwandfrei funktioniert. Achten Sie jedoch darauf, dass solche Testdateien nicht auf Ihren Servern verbleiben. Sie stellen ein enormes Sicherheitsrisiko für Ihre Infrastruktur dar.
...
```
Die `php-fpm`-Konfigurationsdateien sind ausführlich kommentiert. 
