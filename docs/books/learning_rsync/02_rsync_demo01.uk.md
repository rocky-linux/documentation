...
`rsync` має виконати автентифікацію користувача перед синхронізацією даних. **Існує два методи протоколу для автентифікації: протокол SSH та протокол rsync (порт протоколу rsync за замовчуванням – 873)**
...
Перед конкретною демонстрацією синхронізації rsync вам потрібно скористатися командою `rsync`. У Rocky Linux 8 пакет rsync rpm встановлено за замовчуванням, а його версія — 3.1.3-12, як показано нижче:
...
Особисте використання автором: `rsync -avz original location target location`
...
Спочатку перевірте права доступу до каталогу /rsync/. Очевидно, що дозволу "w" немає. Ми можемо використовувати `setfacl` для надання дозволу:
