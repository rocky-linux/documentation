...
Чому б не використовувати RPM-пакет inotify-tools з репозиторію EPEL? А як використовувати вихідний код для компіляції та встановлення?\*\*
...
Запишіть деякі параметри та значення до **/etc/sysctl.conf**, приклади наведені нижче. Потім використовуйте `sysctl -p`, щоб файли набули чинності
...
- **inotifywait**: для безперервного моніторингу, результати виводу в режимі реального часу. Зазвичай він використовується з інструментом інкрементального резервного копіювання rsync. Оскільки це моніторинг файлової системи, його можна використовувати зі сценарієм. Конкретний сценарій ми представимо пізніше.

- **inotifywatch**: для короткострокового моніторингу виводьте результати після виконання завдання.
...
Щодо входу в систему без пароля за протоколом SSH, будь ласка, зверніться до [rsync password-free authentication login](05_rsync_authentication-free_login.md), який тут не описаний. Нижче наведено приклад вмісту сценарію bash. Ви можете додати різні параметри після команди відповідно до ваших потреб. Наприклад, ви також можете додати `--delete` після команди `rsync`.
