...
!!! note "Attention!"
     **rsync itself is only an incremental backup tool and does not have the function of real-time data synchronization(it needs to be supplemented by other programs).  In addition to this, synchronization is one-way. If you want to realize two-way synchronization, you need to cooperate with other tools.**
