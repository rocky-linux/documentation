...
Zu guter Letzt muss ich noch Vates und XCP-ng erwähnen, die auf dem Xen-Hypervisor und der Xen-Orchestra-Web-Administrationsoberfläche basieren. Nach Ansicht des Autors handelt es sich hierbei um Lösungen, die man im Auge behalten sollte, da sie auf soliden Grundlagen aufgebaut sind und aktiv weiterentwickelt werden (`Vates` wird von einem französischen Unternehmen entwickelt). Beachten Sie jedoch, dass Sie bei Vates mehr Geld ausgeben müssen, um auf hyperkonvergente Speicherlösungen (XOSTOR) oder Backup-Lösungen (XO Proxy) zugreifen zu können.
