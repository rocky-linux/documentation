...
1. Ось наш другий пункт у списку. Оскільки ви використали відступ у чотири (4) пробіли (вище), він відображатиметься з такою послідовністю нумерації (2). Якби ви ввели елемент 1 без відступу (у наступному абзаці та коді), то він знову відображався б як елемент 1, а це не те, що вам потрібно.
...
1. Ось наш другий пункт у списку. Оскільки ви використали відступ у чотири (4) пробіли (вище), він відображатиметься з наступною послідовністю нумерації (2). Однак, якби ви ввели елемент 1 без відступу (у наступному абзаці та коді), то він знову відображався б як елемент 1, а це не те, що вам потрібно.
```


!!! tip "Нумерація нумерованого списку не обов'язково відповідає фактичній відображеній кількості."

    Під час включення нумерованого (впорядкованого) списку автор природно схильний нумерувати їх по порядку, але Markdown фактично виконує це завдання самостійно, і тому нумерація по порядку не впливає на фактично відображену кількість. Важливим є форматування (пробіли, відступи тощо). Наприклад, ці два списки функціонально однакові:
    ...
    Ось як вони обидва відображаються в markdown:
    ...
    Щоб візуально ускладнити вигляд, деякі редактори можуть видаляти додані вручну числа та замінювати їх на «1», АЛЕ якщо ви редагуєте документ і додаєте елементи нумерованого списку, той самий редактор буде зручно нумерувати їх послідовно, щоб ви бачили правильне число. Після збереження ці редактори видалять послідовні номери та замінять їх тим самим числом «1».

    Майте на увазі, що якщо ви побачите те, що, на вашу думку, є видаленням нумерованих послідовностей списків автором, це могло бути спричинено редактором, який він використовує, і, ймовірно, не вплине на відображену нумеровану послідовність.

    Як ще один доказ цього, коли автор переніс цей документ, щоб додати цю примітку, редактор видалив нумеровані елементи списку, показані у верхній частині цього розділу, та замінив їх усі на 1. Вони все ще правильно відображаються як 1, 2, 3 тощо.
...
| Тип      | Опис                                                |
|-----------|------------------------------------------------------------|
| note      | відображає текст у синьому полі                                |
| abstract  | відображає текст у світло-блакитному полі                          |
| info      | відображає текст у синьо-зеленому полі                          |
| tip       | відображає текст у синьо-зеленому полі (значок трохи зеленіший) |
| success   | відображає текст у зеленому полі                              |
| question  | відображає текст у світло-зеленому полі                        |
| warning   | відображає текст у помаранчевому полі                             |
| failure   | відображає текст у світло-червоному полі                           |
| danger    | відображає текст у червоному полі                                |
| bug       | відображає текст у червоному полі                                 |
| example   | відображає текст у фіолетовому полі                              |
| quote     | відображає текст у сірому полі                                |
| custom^1^ | завжди відображає текст у синьому полі                        |
| custom^2^ | відображає текст у полі кольором вибраного типу   |
...
!!! note "примітка"
    ...
    1. Або ви додаєте нумерований список із кількома блоками коду:
...
1. Але цей пункт є дуже важливим *і* містить кілька абзаців
...
1. Якщо ви відчуваєте себе перевантаженими, подумайте про:
...
1. Є більше прикладів, але наведене вище повинно показати, що все може бути вкладено всередину. Просто запам’ятайте чотири (4) магічних місця.
