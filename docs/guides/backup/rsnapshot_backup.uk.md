...
`rsnapshot` використовує `rsync` і повністю написаний на Perl без бібліотечних залежностей. Жодних незвичайних вимог до встановлення не існує. У випадку Rocky Linux ви можете встановити `rnapshot` за допомогою репозиторію EPEL. Якщо бажаєте, тут є варіант встановлення з вихідного коду.
...
Нарешті, перейдіть до розділу `### ТОЧКИ РЕЗЕРВНОГО КОПІЮВАННЯ / СЦЕНАРІЇ ###` та додайте будь-які каталоги, які ви хочете додати, у розділ `# LOCALHOST`, не забудьте використовувати ++tab++, а не ++space++ між елементами.
