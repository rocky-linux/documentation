...
Версія ядра, яку ми збираємося створити в наступному розділі, — це версія 5.16.9, яка доступна за адресою:
...
3. Далі, з метою демонстрації, ми додамо підтримку NTFS у наше спеціальне ядро.
...
4. У розділі файлових систем DOS/FAT/NT перейдіть до пункту підтримки файлової системи NTFS.
    ...
    Введіть M (великий регістр), щоб модулі підтримували файлову систему NTFS.
    ...
    А символ зірочки в кутових дужках, <*>, вказує на те, що підтримка цієї функції буде безпосередньо вбудована в ядро. Зазвичай ви можете перемикати всі можливі параметри за допомогою пробілу на клавіатурі.
...
5. Нам потрібно встановити модулі, оскільки ми скомпілювали частини ядра як модулі (наприклад, модуль NTFS). Введіть наступне:
    ...
    У нашій системі Rocky ця команда встановить усі скомпільовані модулі ядра в /lib/modules/<new_kernel-version> каталозі. У цьому прикладі цей шлях буде перекладено на /lib/modules/5.16.9-custom/. Це шлях, з якого ядро буде завантажувати всі завантажувані модулі за потреби.

    !!! Підказка

     Слід (розмір) модулів ядра, встановлених за допомогою «make modules_install», може стати досить великим, оскільки модулі містять символи налагодження. У результаті ви можете легко отримати каталог `/lib/modules/5.16.9-custom/` розміром близько 5 Гб!
...
1. Перебуваючи в кореневому каталозі збірки ядра, скопіюйте та перейменуйте файл bzImage у каталог /boot:
    ...
    Тут версія ядра — це номер версії ядра. Для зразка ядра, яке ми використовуємо в цьому посібнику, ім’я файлу буде vmlinuz-5.16.9-custom. Отже, ось точна команда для цього прикладу:
    ...
    !!! Note "Примітка"
     ...
     Рішення назвати образ ядра vmlinuz-5.16.9-custom є дещо довільним. Це зручно, оскільки зображення ядра зазвичай називають vmlinuz, а суфікс номера версії дійсний, якщо у вас є кілька доступних ядер або ядер, які надають певну функціональність (наприклад, vmlinuz-6.50.0-ws).
...
На нашому демонстраційному сервері Rocky на базі EFI, на якому працює GRUB 2 за допомогою BLS, у файлі завантажувача, розташованому тут, створюється новий запис завантаження: `/boot/loader/entries/6fa25ca775f64accb0d3e53f0e4e6e92-5.16.9-custom.conf`
...
Наступним етапом є тестування нового ядра, щоб переконатися, що система може завантажуватися з ним.
