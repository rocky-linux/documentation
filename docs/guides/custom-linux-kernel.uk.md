...
!!! warning "Перезбірка ядра не рекомендується та не підтримується для Rocky Linux. Перш ніж намагатися зібрати власне ядро, будь ласка, врахуйте наступне:"
...
Найчастіше, коли люди кажуть **linux**, вони зазвичай мають на увазі «дистрибутив Linux» – наприклад, Rocky Linux та Debian – це типи дистрибутивів Linux. Дистрибутив містить усе необхідне для існування Linux як функціональної операційної системи.
Дистрибутиви використовують код із різних проектів з відкритим кодом, які не залежать від Linux.

Linux - це ядро. Ядро лежить в основі [операційної системи].

Єдине, що є більш фундаментальним, ніж ядро, це апаратне забезпечення системи.
Хоча ядро є невеликою частиною повного дистрибутива Linux, воно є найважливішим елементом. Якщо ядро дає збій або виходить з ладу, решта системи йде разом з ним.

## Вихідний код ядра

Дистрибутив Rocky Linux має вихідний код конкретної версії ядра, який він підтримує, доступний у тій чи іншій формі. Вони можуть бути у формі скомпільованого бінарного файлу (`*.src.rpm`), RPM-файлу з вихідним кодом (`*.srpm`) або подібного.
...
1. Спочатку завантажте вихідний код ядра у ваш поточний робочий каталог за допомогою наступної команди `curl`. Впишіть:
...
The first step in building the kernel is configuring its features. Зазвичай список бажаних функцій базуватиметься на апаратному забезпеченні, яке вам потрібно підтримувати. Це, звичайно, означає, що вам знадобиться список цього обладнання.

У системі, яка вже працює під керуванням Linux, ви можете виконувати такі команди, як – `lspci`, `lshw` тощо, щоб показати детальну інформацію про точну конфігурацію обладнання у вашій системі. У дистрибутивах на основі RPM ці утиліти надаються пакетами `pciutils*.rpm` та `lshw*.rpm`.
...
Ми можемо почати фактичну конфігурацію з приблизного уявлення про типи обладнання та функції, які має підтримувати наше нове ядро. But first, some background information.

Дерево вихідних кодів ядра Linux містить кілька файлів під назвою **Makefile** (makefile — це просто текстовий файл із директивами, який також описує зв’язки між файлами в програмі).

These makefiles help to glue together the thousands of other files that make up the kernel source. Для нас тут важливіше те, що make-файли також містять цілі. Цілі — це команди або директиви, які виконує програма make.
...
:   Ця ціль очищає середовище збірки від будь-яких застарілих файлів та залежностей, які могли залишитися від попередньої збірки ядра. Усі попередні конфігурації ядра буде очищено (видалено) із середовища збірки.

`make clean`

:   Ця ціль не виконує свою роботу так ретельно, як ціль mrproper. Він видаляє лише більшість створених файлів. Він не видаляє файл конфігурації ядра (.config).
...
:   Цей інструмент/ціль налаштування ядра на основі графічного інтерфейсу спирається на бібліотеки графічної розробки Qt. Програми на основі KDE/Plasma використовують ці бібліотеки.

`make gconfig`

:   Це також інструмент/ціль налаштування ядра на основі графічного інтерфейсу, але він спирається на інструментарій GTK+. Цей набір інструментів GTK активно використовується в настільному світі GNOME.
...
Нам потрібно перейти (`cd`) до каталогу з вихідним кодом ядра, після чого ми можемо розпочати налаштування ядра.
Але перед початком фактичної конфігурації ядра вам слід очистити (підготувати) середовище збірки ядра за допомогою команди make mrproper:
...
Далі ми почнемо налаштовувати ядро серії Linux 6.\*. Щоб дослідити деякі нутрощі цього процесу, ми ввімкнемо підтримку функції, яку будемо вважати обов’язковою в системі. Once you understand how this works, you can apply the same procedure to add support for any new kernel feature you want. Зокрема, ми ввімкнемо підтримку файлової системи NTFS у нашому спеціальному ядрі.

Більшість сучасних дистрибутивів Linux постачаються з файлом конфігурації ядра для запущеного ядра, доступним у локальній файловій системі у вигляді стисненого або звичайного файлу. У нашій системі Rocky цей файл знаходиться в каталозі `/boot` і зазвичай має назву щось на кшталт `config-*`.
...
The following steps cover how to configure the kernel. Ми будемо використовувати текстову утиліту конфігурації ядра, яка дозволить вам слідкувати за вашим терміналом, незалежно від того, використовуєте ви робоче середовище GUI чи ні.
...
1. Для початку ми скопіюємо та перейменуємо існуючий файл конфігурації з каталогу `/boot` у наше середовище збирання ядра:
   ...
   Тут ми використовуємо `uname -r`, щоб отримати файл конфігурації для працюючого ядра. Команда `uname -r` виводить версію запущеного ядра. Його використання допомагає гарантувати, що ми отримуємо саме ту версію, яку нам потрібно, на випадок наявності інших версій.

   !!! NOTE "Примітка"

        ```
         Редактор конфігурації ядра Linux починає явно шукати та генерувати файл з назвою `.config` (вимовляється «крапка конфіг») у корені дерева вихідних кодів ядра. Цей файл прихований.
        ```
...
2. Запустіть утиліту налаштування графічного ядра:
   ...
   У верхній частині показано корисну інформацію, комбінації клавіш і легенди, які допоможуть вам орієнтуватися в програмі.
...
4. У розділі файлових систем DOS/FAT/NT перейдіть до пункту підтримки файлової системи NTFS.
   ...
   Введіть ++M++ (великі літери), щоб увімкнути модулі для підтримки файлової системи NTFS.

   Використовуйте клавіші зі стрілками, щоб перейти вниз до підтримки запису NTFS (New), а потім натисніть y, щоб включити її.

   Використовуйте клавіші зі стрілками, щоб перейти вниз до підтримки запису в NTFS, і натисніть ++y++, щоб включити її.
   Коли ви закінчите, поруч із кожним параметром має з’явитися літера `M` або символ зірочки (`*`), як показано тут:
...
9. Нам потрібно виконати ще кілька налаштувань нашого дистрибутива Rocky. Впишіть:
   ...
   !!! tip "Підказка"
            ...
            Щоб переглянути результати деяких змін, внесених за допомогою інструмента menuconfig, скористайтеся утилітою grep для перегляду безпосередньо збереженого файлу .config. Наприклад, щоб переглянути вплив підтримки файлової системи NTFS, яку ми ввімкнули раніше, введіть таку команду:
            `         $ grep NTFS ~/build/kernel/.config          CONFIG_NTFS_FS=m          CONFIG_NTFS_DEBUG=y          CONFIG_NTFS_RW=y         `

   !!! NOTE "Коротка примітка про модулі ядра"

        ```
         Підтримка завантажуваних модулів — це функція ядра Linux, яка дозволяє динамічне завантаження (або видалення) модулів ядра.    
        
         Модулі ядра — це фрагменти скомпільованого коду, які можна динамічно вставляти в запущене ядро, а не вбудовувати в нього постійно. Таким чином, можна вмикати функції, які не часто використовуються, але вони не займатимуть місце в пам'яті, коли не використовуються.
        
         На щастя, ядро ​​Linux може автоматично визначати, що і коли завантажувати. Звичайно, не кожну функцію можна компілювати як модуль. Ядро повинно знати кілька речей, перш ніж воно зможе завантажувати та вивантажувати модулі, наприклад, як отримати доступ до жорсткого диска та проаналізувати файлову систему, де зберігаються завантажувані модулі. Деякі модулі ядра також зазвичай називають драйверами.
        ```
...
The final customization will be to add an extra piece of information used in the final name of our kernel. This will help us be able to differentiate this kernel from any other kernel with the same version number. Ми додамо тег «custom» до інформації про версію ядра. Це можна зробити шляхом редагування основного Makefile і додавання потрібного тегу до змінної EXTRAVERSION.

The compilation stage of the kernel-building process is by far the easiest, but it also takes the most time. Все, що потрібно на цьому етапі, це просто виконати команду make, яка потім автоматично згенерує та вирішить будь-які проблеми залежностей, скомпілює саме ядро та скомпілює будь-які функції (або драйвери), які були активовані як завантажувані модулі.
...
1. По-перше, ми додамо додаткову частину до ідентифікаційного рядка для ядра, яке збираємося створити. Перебуваючи в корені дерева вихідних кодів ядра, ми скористаємося утилітою `sed` для редагування Makefile на місці. Змінна, яку ми хочемо змінити, знаходиться близько до верхньої частини файлу.
   Ми хочемо змінити рядок у файлі, який виглядає так:
   ...
   Використайте наступну команду `sed`, щоб внести зміни. Впишіть:
...
2. Передайте ціль kernelversion команді make, щоб переглянути повну версію ядра, яку ви щойно налаштували:
   ...
   !!! Tip "Порада"
        ...
        ```
         Ви можете скористатися всією цією додатковою обчислювальною потужністю (процесорами, ядрами тощо) на більшості сучасних систем і значно пришвидшити операції, що ресурсомісткі для процесора, такі як компіляція ядра. Для цього ви можете передати команді make параметр, який вказує кількість завдань, що виконуються одночасно. Зазначена кількість завдань потім розподіляється та виконується одночасно на кожному ядрі процесора. Синтаксис команди такий:
        
             ```bash
             make -j N
             ```
             де N – кількість завдань, що виконуються одночасно. Наприклад, якщо у вас є процесор з восьми (8) ядрами, ви можете ввести:
...
5. Нам потрібно встановити модулі, оскільки ми скомпілювали частини ядра як модулі (наприклад, модуль NTFS). Введіть наступне:
   ...
   У нашій системі Rocky ця команда встановить усі скомпільовані модулі ядра в каталог `/lib/modules/<new_kernel-version>`. У цьому прикладі цей шлях буде перетворено на `/lib/modules/6.5.7-custom/`. Це шлях, з якого ядро буде завантажувати всі завантажувані модулі за потреби.

   !!! tip "Підказка"

            Розмір модулів ядра, встановлених за допомогою `make modules_install`, може бути досить значним, оскільки модулі містять символи налагодження. В результаті ви можете легко отримати каталог `/lib/modules/6.5.7-custom/` розміром майже 5 ГБ!
           
            У цьому посібнику ми уникаємо такого великого розміру, додавши опцію `INSTALL_MOD_STRIP=1` у наш виклик `make modules_install`. Ви можете зменшити загальний розмір на порядки (наприклад, менше ніж 200 МБ!!), видаливши ці символи налагодження.
           
            Це можна зробити, додавши опцію `INSTALL_MOD_STRIP=1` до команди `make modules_install`.
...
Відповідний файл карти для цього буде розташований за адресою `~/build/kernel/System.map`. You will need both files for the install phase.
...
`System.map` дозволяє Linux перетворювати ці шістнадцяткові числа на читабельні імена, що спрощує налагодження. Although this is mainly for the benefit of developers, it can be handy when you are reporting a problem.
...
1. Перебуваючи в кореневому каталозі збирання ядра, скопіюйте та перейменуйте файл `bzImage` у каталог `/boot`:
   ...
   Тут `<kernel-version>` — це номер версії ядра. Ім'я файлу для прикладу ядра, яке ми використовуємо в цьому посібнику, — `vmlinuz-6.5.7-custom`. Отже, ось точна команда для цього прикладу:
   ...
   !!! Note "Примітка"
        ...
        ```
         Рішення назвати образ ядра `vmlinuz-6.5.7-custom` є дещо довільним. Це зручно, оскільки образи ядра зазвичай називають vmlinuz, а суфікс номера версії корисний, коли у вас є кілька доступних ядер або ядра, що забезпечують певну функціональність (наприклад, `vmlinuz-6.50.0-ws`).
        ```
...
3. З ядром, файлом `System.map` та модулями, ми готові до останнього кроку. Синтаксис необхідної команди:

    ```bash
    kernel-install add <kernel-version> <kernel-image>
    ```

   Тут `<kernel-version>` — це номер версії (і назва) ядра. А `<kernel-image>` — це шлях до щойно скомпільованого образу ядра.
...
Команда `kernel-install` — це зручний скрипт оболонки. Він може бути доступний не в кожному дистрибутиві Linux, але він доступний у новіших дистрибутивах Fedora, RHEL, CentOS. Цей інструмент автоматизує багато останніх ручних дій, які нам зазвичай доводиться робити, щоб налаштувати систему для завантаження нового ядра, яке ми щойно створили.

In particular, the tool does the following:

- Він створює відповідний початковий образ файлової системи RAM (образ initramfs, тобто файл `/boot/initramfs-<kernel-version>.img`).
  Щоб зробити це вручну на системах, де `kernel-install` недоступний, скористайтеся командою `mkinitramfs`.
- Він запускає команду `depmod` (яка створює список залежностей модуля).
- It updates the boot loader configuration.

Для систем, що працюють з новішими версіями GRUB2, файл матиме назву `/boot/grub2/grub.cfg`. Для систем на основі EFI також оновлено файл `/boot/efi/<distro>/fedora/grub.cfg`.

А для систем, що працюють зі старими версіями GRUB, це буде файл `/boot/grub/grub.conf` або `/boot/grub/menu.lst`. А для зовсім нових дистрибутивів, які реалізували нову специфікацію завантажувача (BLS), новий запис завантажувача буде додано до каталогу `/boot/loader/entries/` або до будь-якого каталогу, на який вказує змінна з назвою `blsdir`.
