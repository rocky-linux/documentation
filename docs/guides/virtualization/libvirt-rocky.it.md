...
- Add your user to the `libvirt` group. Ciò consente di gestire le macchine virtuali e di utilizzare comandi come `virt-install` come utente non root:
...
- You can create a virtual machine on the command line by using the `virt-install` command. Ad esempio, per creare una macchina virtuale Rocky Linux 9.5 Minimal, si deve eseguire il seguente comando:
