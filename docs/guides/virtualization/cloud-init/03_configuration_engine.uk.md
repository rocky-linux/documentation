...
Коли `cloud-init` бачить заголовок `#cloud-config`, він інтерпретує файл як набір інструкцій у форматі YAML. Ключі верхнього рівня в цьому YAML-файлі безпосередньо пов'язані з модулями `cloud-init`.
