...
У демонстраційній системі цього посібника ми приймаємо значення за замовчуванням (_English US_) і не вносимо жодних змін.
...
Коли ви закінчите з цим екраном, натисніть кнопку ++"done"++.

#### Підтримка мови

Опція `Language Support` на екрані _Installation Summary_ дозволяє вказати підтримку додаткових мов.
...
У розділі _Software_ на екрані _Installation Summary_ ви можете вибрати або змінити джерело встановлення, а також додаткові пакети (програми), які встановлюються.
...
Виберіть опцію _Minimal Install_ (базова функціональність).
...
У розділі _Storage Configuration_:

1. Виберіть перемикач ++"Automatic"++.

2. Натисніть ++"done"++ у верхній частині екрана.

3. Щойно програма встановлення визначить, що у вас є вільний диск, вона повернеться до екрана _Installation Summary_.
...
Натисніть кнопку ++"готово"++, щоб повернутися до головного екрана _Installation Summary_.
...
1. Клацніть поле __Root Password_ у розділі _User Settings_, щоб розпочати екран завдання _Root Password_.
...
2. У текстовому полі _Root Password_ встановіть надійний пароль для користувача root.

3. Введіть той самий пароль ще раз у текстове поле _Confirm_.
...
2. Заповніть поля на екрані _Create User_, вказавши таку інформацію:
...
Щойно ви будете задоволені вибраними параметрами для різних завдань встановлення, натисніть кнопку ++"Розпочати встановлення"++ на головному екрані _Installation Summary_.
...
2. У запиті пароля введіть `04302021` (пароль Rockstar) і натисніть ++enter++ (пароль **не** відображатиметься на екрані, це нормально).
