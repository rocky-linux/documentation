...
IPv6 n'a pas besoin d'être présenté, mais au cas où vous ne le sauriez pas, IPv6 remplace le protocole IPv4 qui utilise des adresses hexadécimales 128 bits au lieu d'adresses décimales 32 bits.
...
En raison de l'épuisement des adresses IPv4, un besoin d'un espace d'adressage IP étendu est apparu et résolu sous la forme d'IPv6. Cependant, de nombreux réseaux ne prennent toujours pas en charge IPv6 en raison de l’omniprésence de la traduction d’adresses réseau (NAT). C'est pour cette raison que la société Hurricane Electric propose des tunnels IPv6.

## Prérequis

- Un [tunnel IPv6 gratuit de Hurricane Electric](https://tunnelbroker.net/)

- Un serveur Rocky Linux avec une adresse IP publique et un protocole de messages de contrôle Internet (ICMP) non filtré.
...
Entrez ensuite votre adresse IPv4 publique, sélectionnez l'emplacement de votre point de terminaison et cliquez sur **Créer un tunnel**.
