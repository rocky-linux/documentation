...
## Determine the latest PHP

To determine the latest available PHP version, use:
...
You will note that the default version of a Rocky 8 is 7.2 that has already reached its end of life at the time of this writing.
...
First, install PHP in CGI mode. You can only make it work with the Apache web server and its `mod_php` module. You will see, later in this document, how to integrate PHP in Nginx and Apache.
...
The example shown installs PHP with the modules commonly installed with it:
...
The main configuration file is in the `/etc/php-fpm.conf` file:
...
In static mode, the value of `pm.max_children` sets the number of child processes:
...
To activate the page, set its access path by the `pm.status_path` directive:
...
The `slowlog` directive specifies the file that receives logging of requests that are too long (for example, files whose time exceeds the value of the `request_slowlog_timeout` directive).
...
A server with 8 gigabytes of RAM, keeping 1 gigabyte for the system and 1 gigabyte for the OPCache (see the rest of this document), leaves 6 gigabytes to process PHP requests from clients.
...
* `opcache.memory_consumption` corresponds to the amount of memory needed for the `opcache` (you increase this until you obtain a correct hit ratio).
