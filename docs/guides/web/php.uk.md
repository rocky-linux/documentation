...
* Можливість кращого **розбиття програм**: запуск процесів з різними uid/gid, з персоналізованими файлами php.ini,
...
Встановлення php є відносно тривіальним, оскільки воно складається з встановлення основного пакета та кількох модулів php, які вам знадобляться.
...
Vhost за замовчуванням має працювати з коробки. PHP надає функцію `phpinfo()`, яка генерує зведену таблицю його конфігурації. Це дуже корисно для перевірки правильної роботи php. Однак будьте обережні, щоб не залишати такі тестові файли на своїх серверах; вони становлять величезний ризик для безпеки вашої інфраструктури.
...
Як ми зазначали раніше в цьому документі, є багато переваг перемикання веб-хостингу в режим php-fpm.
...
| Інструкції | Опис                                                                                                                         |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `[pool]`   | Ім'я пулу процесів. Конфігураційний файл може складатися з кількох пулів процесів (ім'я пулу в дужках починає новий розділ). |
...
!!! Note "Примітка"
    ...
    Використання сокета, коли веб-сервер і PHP-сервер знаходяться на одній машині, усуває рівень TCP/IP і оптимізує продуктивність.
...
Під час роботи через сокет вам потрібно налаштувати `listen.allowed_clients`, щоб обмежити доступ до php-сервера певними IP-адресами.
...
Важливо оптимізувати кількість запитів, які можна буде обслуговувати, проаналізувати пам’ять, яку використовують PHP-скрипти, і таким чином оптимізувати максимальну кількість запущених потоків.
...
* кількість php-скриптів для кешу (кількість ключів + максимальна кількість скриптів)
