...
Натисніть <kbd>ENTER</kbd>. Ви все ще маєте бути в режимі вставки, тому наступним кроком є додавання вашого запису. Як показано в `crontab` з порожнім коментарем (вище), це **m** для хвилин, **h** для годин, **dom** для дня місяця, **mon** для місяця та **dow** для дня тижня.
