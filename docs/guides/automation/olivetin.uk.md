...
Звичайно, технічно ви могли б створити щось подібне самостійно, з нуля, маючи достатній досвід програмування... але це _набагато_ простіше. Після налаштування це виглядає приблизно так (зображення надано з [репозиторію OliveTin](https://github.com/OliveTin/OliveTin)):
...
OliveTin може працювати як звичайна служба `systemd`, але поки що не вмикайте її. Спочатку вам потрібно налаштувати файл конфігурації.
...
Роблячи це, ви можете запускати будь-яку програму, керувати віддаленими машинами за допомогою SSH, запускати веб-хуки тощо. Перегляньте [офіційну документацію](https://docs.olivetin.app/action_examples/intro.html) для отримання додаткових ідей.
...
На передній частині це виглядає так (і так, OliveTin має темний режим, і мені _справді_ потрібно змінити цей значок):
