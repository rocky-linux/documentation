...
자동화된 무작위 실행 시간이 [위의 워크스테이션](#_5)에에서 잘 작동하지 않거나  [위의 서버](#_4)에서 예약된 시간이 제대로 작동하지 않는 경우 직접 cron을 생성할 수 있습니다. 이 예제에서는 root 사용자로 작업하는 것을 전제로 합니다. 이를 위해 [가정](#_2)를 보고 다음을 입력합니다:
