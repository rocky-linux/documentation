...
- Розуміння того, як використовувати ваш улюблений редактор для зміни файлу конфігурації (наприклад, _vim_) у середовищі командного рядка.
...
`anacron` виконує команди регулярно, а частота роботи вимірюється одиницями днів. Він підходить для комп’ютерів, які не працюють у режимі 24/7, наприклад ноутбуків і настільних ПК. Припустимо, у вас заплановано виконання завдання (наприклад, сценарію резервного копіювання) рано вранці кожного дня за допомогою `crontab`. Коли ви засинаєте, ваш робочий стіл або ноутбук вимкнено. Ваш сценарій резервного копіювання не запускається. Однак, якщо ви використовуєте `anacron`, можете бути впевнені, що наступного разу, коли ви ввімкнете настільний комп’ютер або ноутбук, скрипт резервного копіювання запуститься.\*\*
...
**/etc/cron.hourly/** -За допомогою `journalctl -u crond.service` можна дізнатися, що файли, що зберігаються всередині, насправді викликаються `crond`, а це означає, що команда виконуватиметься після першої хвилини кожної години. Таким чином:
...
Щоб певні файли виконувалися протягом цих автоматично визначених часових проміжків, вам потрібно лише скопіювати файл скрипта до відповідного каталогу та перевірити, чи має він **дозвіл на виконання (chmod +x)**. Тому вам потрібно лише дозволити системі автоматично запускати сценарій в один із запланованих часів, що спрощує завдання автоматизації.
