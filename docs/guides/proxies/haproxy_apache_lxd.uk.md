...
Змініть екран привітання, запустивши `httpd`. Цей екран з’являється, коли веб-сайт не налаштовано, по суті це сторінка за замовчуванням, яка завантажується. У Rocky Linux ця сторінка розташована тут `/usr/share/httpd/noindex/index.html`. Зміна цього файлу не вимагає прямого доступу до контейнера. Просто виконайте наступне:
