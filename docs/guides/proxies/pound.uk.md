...
- Знайомство з редактором командного рядка. Автор використовує тут `vi` або `vim`, але замініть їх вашим улюбленим редактором.
...
- Хоча в цьому документі містяться всілякі твердження щодо `https`, цей посібник стосується лише сервісу `http`. Щоб правильно використовувати `https`, вам потрібно буде налаштувати ваш сервер pound за допомогою справжнього сертифіката від справжнього центру сертифікації.
...
- Розділ "ListenHTTP" представляє службу `http` (порт 80) та "адресу", яку проксі-сервер прослуховуватиме. Вам необхідно змінити це на фактичну IP-адресу нашого сервера Pound.
- Розділ "ListenHTTPS" представляє службу `https` (порт 443) та "адресу", яку проксі-сервер прослуховуватиме. Необхідно змінити це на IP-адресу сервера Pound.
- Опція «Cert» — це самопідписаний сертифікат, наданий процесом встановлення Pound. Ви хочете замінити це у виробничому середовищі справжнім сертифікатом, використовуючи одну з цих процедур: [Генерація ключів SSL](../security/ssl_keys_https.md) або [Ключі SSL за допомогою Let's Encrypt](../security/generating_ssl_keys_lets_encrypt.md).
...
У вашому файлі `pound.conf` можна використовувати _багато_ опцій, включаючи директиви повідомлень про помилки, опції журналювання, значення часу очікування тощо. Ви можете знайти доступні опції [за цим посиланням.] (https://linux.die.net/man/8/pound)
