...
* Rocky Linux Minimal ISO image (DVD is not required).
* `kickstart` file to apply to the ISO
* To learn more about `kickstart` files, along with included examples and how to apply them to a Rocky Linux ISO, please check out the [Kickstart Files and Rocky Linux](<URL_HERE>) guide. 
...
* The `base os` repository will be downloaded locally, along with all of its packages:
...
Once your own ISO is built, it makes it much easier to deploy single or hundreds of machines with the one image.
