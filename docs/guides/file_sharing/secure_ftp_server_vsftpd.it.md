...
Salvare la configurazione. (++shift+colon+w+q++ per `vi`.)
...
L'utente e la password sono separati da una riga; inserire l'utente, premere ++enter++ e inserire la password. Continuate fino a quando non avrete aggiunto tutti gli utenti a cui volete dare accesso al sistema. Esempio:
...
- L'*hash* è il metodo di accesso sottostante che si sta specificando
- L'opzione -f indica di leggere da un file specificato.
- Il file specificato è *vusers.txt*
- E il database che si sta creando o aggiungendo è *vsftpd-virtual-user.db*
...
Salvare le modifiche e uscire (++shift+colon+w+q++ in `vi`).
...
(Fare riferimento alla sezione "Impostazione degli utenti virtuali")

Per cambiare la proprietà della directory, digitate questo comando alla riga di comando:
...
Quando si esegue il test con un utente virtuale sul server che esegue `vsftpd`, si ottiene un messaggio di fiducia del certificato SSL/TLS. Questo messaggio di fiducia informa la persona che il server utilizza un certificato e le chiede di approvarlo prima di continuare. È possibile inserire i file nella cartella "local_root" quando si è connessi come utente virtuale.

Se non si riesce a caricare un file, potrebbe essere necessario tornare indietro e verificare nuovamente tutti i passaggi. Ad esempio, potrebbe essere che i permessi di proprietà per "local_root" non siano impostati sull'utente "vsftpd" e sul gruppo "nogroup".

## Conclusione

`vsftpd` è un server FTP molto diffuso e comune e può essere un server indipendente o parte di un [server Web rigido Apache](../web/apache_hardened_webserver/index.md). È abbastanza sicuro se impostato per utilizzare utenti virtuali e un certificato.
