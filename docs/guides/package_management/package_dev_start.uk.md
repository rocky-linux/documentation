...
Попередній процес rockybuild створить деякі файли журналу, які можна використовувати для налагодження невдалих збірок програми. Результати та/або журнали процесу збірки зберігаються у файлі `~/rocky/builds/<PACKAGE NAME>/r8`. Наприклад, `~/rocky/builds/sed/r8`
