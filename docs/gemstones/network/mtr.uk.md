...
`mtr` — це інструмент діагностики мережі, який може діагностувати проблеми з мережею. Він використовується для заміни команд `ping` та `traceroute`. Що стосується продуктивності, команда `mtr` є швидшою.
...
Типові опції команди `mtr` такі. За звичайних обставин не потрібні додаткові параметри, а потім безпосередньо ім’я хоста або IP-адреса:
