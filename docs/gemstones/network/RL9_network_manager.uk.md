...
У 2004 році Red Hat запустила проект **NetworkManager**, метою якого є спрощення для користувачів Linux задоволення потреб поточного управління мережами, особливо управління бездротовими мережами. Сьогодні проєктом керує GNOME. Домашню сторінку NetworkManager можна знайти [тут](https://networkmanager.dev/).
...
Для RockyLinux 8.x ми представили, як налаштувати його мережу [у цьому документі](./nmtui.md). Ви можете використовувати `vim` для редагування файлу конфігурації мережевої карти в каталозі **/etc/sysconfig/network-script/**, або ж `nmcli`/`nmtui`, обидва варіанти прийнятні.
