...
`NetworkManager` — це стандартний набір інструментів для налаштування мережі Linux, який підтримує серверні та настільні середовища. Зараз більшість популярних дистрибутивів підтримують його. Цей набір інструментів налаштування мережі підходить для Rocky Linux 8 і новіших версій. Якщо ви хочете налаштувати мережеву інформацію графічно (тобто за допомогою командного рядка `nmtui`), вам потрібно зробити лише ось що:
...
Ви можете скористатися клавішею ++Tab++ або ++стрілка вгору++ ++стрілка вниз++ ++стрілка вліво++ ++стрілка вправо++, щоб вибрати конкретне. Якщо ви хочете змінити інформацію про мережу, виберіть **Редагувати підключення**, а потім натисніть ++Enter++. Виберіть іншу мережеву карту та натисніть **Редагувати..** для редагування.

### DHCP IPv4

Для IPv4, якщо потрібно отримати мережеву інформацію за допомогою DHCP, вам потрібно лише вибрати _IPv4 CONFIGURATION_ **&lt;Automatic&gt;** та виконати у терміналі `systemctl restart NetworkManager.service`. У рідкісних випадках потрібно змінити мережеву карту, щоб вона почала діяти. Наприклад, таким чином: <code>nmcli connection down ens33</code>, <code>nmcli connection up ens33</code> Наприклад: `nmcli з'єднання не працює ens33`, `nmcli з'єднання активовано ens33`

### Виправити вручну інформацію про мережу

Якщо ви хочете вручну виправити всю мережеву інформацію IPv4, вам потрібно вибрати **&lt;Manual&gt;** після _КОНФІГУРАЦІЯ IPv4_ та додавати її рядок за рядком. Наприклад, мені подобається таке:
...
Усі дистрибутиви RHEL <font color="red">7.x</font> або <font color="red">8.x</font> налаштовані однаково. Файл конфігурації мережевої інформації зберігається в каталозі **/etc/sysconfig/network-scripts/**, і одній мережевій карті відповідає один файл конфігурації. Файл конфігурації має багато параметрів, як показано в наступній таблиці. Зверніть увагу! Параметри повинні бути написані великими літерами.
