...
Цей bash-скрипт призначений для редагування конфігураційного файлу `/etc/systemd/logind.conf` на сервері або робочій станції Rocky Linux. Зокрема, він змінює опцію `HandleLidSwitch` та встановлює для неї значення `ignore`. Ця зміна конфігурації зазвичай використовується, щоб запобігти призупиненню системи або виконанню будь-яких дій, коли кришка ноутбука закрита.
...
- Припускається, що файл `logind.conf` знаходиться за адресою `/etc/systemd/logind.conf`. Якщо ваша система використовує інше розташування, змініть сценарій відповідно.
