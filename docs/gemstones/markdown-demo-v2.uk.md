...
У цьому посібнику демонструються популярні теги Markdown, що використовуються на [https://docs.rockylinux.org](https://docs.rockylinux.org), та містить тег admonitions, який не є частиною стандартних тегів Markdown.
...
Застереження мають простий синтаксис: блок починається з `!!!`, за яким йде ключове слово, що використовується як кваліфікатор типу. Вміст блоку наводиться в наступному рядку з відступом на чотири інтервали:
...
За замовчуванням заголовок дорівнюватиме кваліфікатору типу в регістрі заголовка. Однак, це можна змінити, додавши рядок у лапках, що містить коректний код Markdown (включаючи посилання, форматування тощо) після кваліфікатора типу:
