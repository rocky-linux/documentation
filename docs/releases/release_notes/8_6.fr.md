...
* Le serveur OpenSSH prend désormais en charge les configurations `drop-in`. Cela signifie que vous pouvez utiliser la directive `Include` dans `sshd_config` similairement à ce qui est maintenant disponible pour `ssh_config`.
