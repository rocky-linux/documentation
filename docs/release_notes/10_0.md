...
Rocky Linux does not support upgrades to any major release. To move from 8.x or 9.x to Rocky Linux 10 (RL 10), you should perform a fresh install.
...
The overview that follows highlights changes in the - Architecture, Kernel, Installer, Networking, Security, Virtualization, Containers, Storage, Package management - categories. Chosen here are the features most likely to have relevance to a general audience.
...
More details about this milestone Rocky Linux 10’s RISC-V support are [found here](https://rockylinux.org/news/rockylinux-support-for-riscv).
...
AMD's implementation of the x86-64-v3 microarchitecture level, introduced with their Excavator microarchitecture, which serves as the baseline for Rocky Linux 10 on x86_64. This means that processors older than those based on the Excavator architecture might not be fully supported by Rocky Linux 10.

!!! Note "Raspberry pi image differences"

    The key difference between the Rocky Linux pi images for version 10, is that 10 supports pi 4 and 5, whereas 9 supports 3, 4, 5 and pi zero2w.
...
* The disabling of the root account by default. System management requires creating an administrative user with full `sudo` privileges. If you opt to enable the root account by setting its password, then you can create standard users without administrative privileges.
...
* There is no-longer a time zone map under the "Time & Date" section of the installer.
...
* The removal of Documentation that was, on other versions, available at the time of installation.
...
The `rh_waived` kernel command-line argument can enable these features if required. The Earliest Eligible Virtual Deadline First (EEVDF) replaces the CFS scheduler, which prioritizes latency-sensitive tasks with shorter time slices to improve responsiveness.
...
You have got to use NetworkManager now. Those tools include `nmcli`, `nmtui` and `nmstate`. This means some older files and commands will not work anymore, and where you keep your network configuration files has changed too.
...
The implementation of the DHCP client in RL 10 is with an internal subsystem of NetworkManager. The legacy dhcp-client package is no longer supported upstream and is removed.

Kea DHCP replaces the end-of-life ISC DHCP server in RL 10.

#### Network Bonding and Teaming

Previous Rocky Linux versions deprecated the NIC teaming feature. It is removed entirely in Rocky Linux 10. As a replacement, configure a bond instead of a network team.

### Storage

In addition to the following changes, a number of storage-related device drivers are added, updated, or removed from RL 10.
...
* Monolitic libvirtd daemon is deprecated. Use the modular daemons and sockets should as a replacement. Please see detailed documentation about the differences in the [upstream documentation](https://libvirt.org/daemons.html#monolithic-vs-modular-daemons)
...
RPM is updated to version 4.19, and includes enhancements and updates to packaging and building tools. There is a new `rpmsort` command introduced for correctly ordering RPM version strings in output pipelines. As an example:
...
There are updates to a number of popular programming languages to more recent versions:
...
There are updates to a number of popular server daemons to more recent versions as well.
...
In certain hardware configurations, such as virtualized environments with enabled 3D graphics, you might experience Anaconda hanging before showing the setup wizard. This issue should be resolvable by switching away from tty6 to another tty and then back to tty6. For example, switch to tty1 and back to tty6 with: ++ctrl+alt+f1++ and then ++ctrl+alt+f6++.
