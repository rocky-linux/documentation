...
Pour une liste complète des changements majeurs, veuillez consulter la liste en amont [ici](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html/9.0_release_notes/overview#overview-major-changes).

!!! warning "Veuillez vérifier la compatibilité du processeur"

    Avec Rocky Linux 9.0, certains anciens processeurs ne fonctionneront pas correctement, ou voire pas du tout. Les architectures suivantes sont considérées comme des exigences minimales pour une installation réussie :
    
    \* AMD et Intel 64-bit architectures (x86-64-v2) [Vérifier la compatibilité CPU en suivant cette procédure](https://docs.rockylinux.org/gemstones/test_cpu_compat/)
...
- Le recours à **SHA-1** comme fonction de hachage cryptographique est déprécié. Les fonctions de hachage cryptographique produites par SHA-1 ne sont plus considérées comme sécurisées.  Il y a quelques exceptions, qui sont détaillées dans le lien amont ci-dessus.
- **OpenSSL** est maintenant en version 3.0.1 avec de nombreuses améliorations, y compris le concept de fournisseur, le nouveau schéma de versions, un client HTTP(S) amélioré, le support de nouveaux protocoles, formats, algorithmes, et plus.
...
- Les performances de **SELinux**, la surcharge mémoire, le temps de chargement et bien d'autres aspects ont été substantiellement améliorés. Il y a toute une série de changements avec plus de détails dans les notes de publication amont.
...
- **mptcpd**, ou démon TCP MultiPath, peut être utilisé à la place de `iproute2` pour configurer les points de terminaison MultiPath TCP.
- NetworkManager utilise désormais des fichiers de clés pour stocker les nouveaux profils de connexion par défaut, mais prend toujours en charge l'utilisation de `ifcfg`.
- **iptables-nft** et **ipset** sont maintenant obsolètes, ce qui inclut aussi les utilitaires, `iptables`, `ip6tables`, `ebtables`, et `arptables`. Ceux-ci sont tous remplacés par le framework `nftables`.
...
Plusieurs applications de programmation dynamique, de contrôle de versions et de gestion d'applications web répertoriées ci-après ont été mises à jour vers de nouvelles versions dans Rocky Linux 9.0.
Pour une liste complète des changements et des descriptions, veuillez consulter [ce lien amont](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html/9.0_release_notes/New-features#enhancement_dynamic-programming-languages-web-and-database-servers).
...
Pour installer Rocky Linux 9.0, rendez-vous sur la [page de téléchargement](https://rockylinux.org/download/) et téléchargez la version dont vous avez besoin pour votre architecture.
Un guide d'installation complet pour Rocky Linux 9.0 est disponible sur le [site de documentation de Rocky Linux ici](https://docs.rockylinux.org/guides/installation/).
...
Optionnellement, vous pouvez accéder aux sommes de contrôle de n'importe quelle version de Rocky Linux en vous rendant sur le [dépôt github ici](https://github.com/rocky-linux/checksums).

## Problèmes Connus

Comme pour toute version, il existe des problèmes qui sont déjà connus. Vous pouvez consulter la liste actuelle des problèmes depuis l'amont [ici](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html/9.0_release_notes/known-issues).

- **Limitation de l'utilisation de l'installation avec l'ISO minimale** - La plupart des paquets liés à la sécurité ne fonctionneront pas. Si vous avez besoin de profils de sécurité/SCAP et des paquets associés au moment de l'installation, veuillez utiliser l'ISO DVD.
