...
- **OpenSSH** 现在是 8.7p1 版本，此版本有许多改进，最明显的是用提供更多可预测文件名处理的 SFTP 协议取代了 SCP/RCP 协议。
...
- **mptcpd** 或 MultiPath TCP 守护程序可替代 `iproute2` 来配置 MultiPath TCP 端点。
- NetworkManager 现在默认使用密钥文件来存储新的连接配置文件，但仍然支持使用 `ifcfg` 。
...
接下来列出的几个动态编程语言、版本控制系统以及 web 相关的应用套件已经在 Rocky Linux 9.0 中更新到了较新的版本。
有关完整的更改清单和描述，请参阅 [这个上游链接](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html/9.0_release_notes/New-features#enhancement_dynamic-programming-languages-web-and-database-servers) 。
...
要安装 Rocky Linux 9.0，请转到 [下载页面](https://rockylinux.org/download/) 并下载您架构所需的版本。
Rocky Linux 9.0的完整安装指南可在 [Rocky Linux 文档网站](https://docs.rockylinux.org/guides/installation/) 上找到。
