...
!!! Bug "Upgrading with LVM devices may result in boot failure"
    ...
    This bug affects both physical and virtual systems, though Libvirt(KVM), VMWare, as well as some other guests appear to not be affected.
    
    To see if you may be impacted by this bug, inspect the contents of `/etc/lvm/devices/system.devices`. If this file exists **and** contains a "sys_wwid"-type device and have several repeated underscores. See the following for an example:
    ...
    If you are impacted by this bug, then until the upstream bug is released by RHEL and Rocky, please follow one of the following options **before** upgrading and rebooting your system:
    ...
    2. Prior to rebooting after upgrading, delete the `/etc/lvm/devices/system.devices` file and once the system has rebooted, run `vgimportdevices --all` to regenerate the file.
    ...
    In the case of a system which has already been upgraded and is in recovery mode, you may simply delete the `/etc/lvm/devices/system.devices` file from recovery mode and reboot the system.
