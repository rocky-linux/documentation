...
!!! Bug "Upgrading with LVM devices may result in boot failure"
    ...
    To see if you may be impacted by this bug, inspect the contents of `/etc/lvm/devices/system.devices`. If this file exists **and** the contents of "sys_wwid" are followed by multiple repeated underscores, you are likely impacted. See the following for an example:
