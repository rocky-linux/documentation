...
!!! Bug "使用 LVM 设备升级可能导致引导失败"
    ...
    1. 在升级了 `lvm2` 软件包之后，但在重启之前，通过运行 `lvmdevices --update` 重新生成 `/etc/lvm/devices/system.devices` 文件。 在重启系统之前，请确认已将 `/etc/lvm/devices/system.devices` 的内容已更改以匹配新格式。
    ...
    如果系统已经升级并处于恢复模式，您只需将 `/etc/lvm/devices/system.devices` 文件移出恢复模式并重启系统即可。 一旦系统重启之后，将以新格式重新生成文件。 有关详细信息，请参阅上面的选项 2 。
...
在已经可用的 Workstation/Workstation Lite/KDE/XFCE/MATE 实时镜像基础上，已在 x86_64 和 aarch64 中添加了 Cinnamon 实时镜像。
...
- 添加了 aarch64 架构的 64k 页大小内核（`kernel-64k`）
- NFS `nfsrahead` 已被添加
- 对 Aspeek GPU 现在默认启用 Wayland
- 英特尔 ARC GPU 理论上现在也可以正常工作了
- 导致 Flatpaks 中所有字体（Cantarell） 显示异常的 Flatpak bug 得到了修复
